...
description: A real-world example of migrating from direct PostgreSQL to Supabase in nexus
category: Examples
---

# Supabase Integration Case Study

This case study describes how a team migrated their nexus application from direct PostgreSQL deployment to Supabase for enhanced scalability and reduced operational overhead.
...
The team built a customer support dashboard using nexus to help their customer service representatives track and resolve support tickets. Initially, they deployed with the default PostgreSQL option for simplicity during development.
...
### Step 5: Configuring nexus for Supabase
...
One advantage of nexus's new automatic database detection is that they could easily switch back to PostgreSQL if needed by simply removing the `SUPABASE_URL` environment variable and ensuring their PostgreSQL settings were configured.
...
nexus's automatic database detection made this migration straightforward, allowing the team to switch between database options without any changes to their application code.
