...
description: nexus 中从直接 PostgreSQL 迁移到 Supabase 的实际案例
category: 案例示例
---

# Supabase 集成案例研究

本案例研究描述了一个团队如何将其 nexus 应用程序从直接 PostgreSQL 部署迁移到 Supabase，以提高可扩展性并减少运营开销。
...
该团队使用 nexus 构建了一个客户支持控制面板，帮助客服人员跟踪和解决支持工单。最初，他们在开发阶段使用了默认的 PostgreSQL 选项以简化部署。
...
nexus 新的自动数据库检测的一个优势是，如果需要，他们可以通过简单地删除 `SUPABASE_URL` 环境变量并确保配置了他们的 PostgreSQL 设置，轻松切换回 PostgreSQL。

## 结论

从直接 PostgreSQL 迁移到 Supabase 使团队能够更专注于应用程序的功能，而不是数据库管理。内置的可扩展性、安全功能和减少的运营开销使其成为不断发展的应用程序的绝佳选择。

nexus 的自动数据库检测使这种迁移变得简单，允许团队在不改变应用程序代码的情况下切换数据库选项。 