---
title: 功能和发布说明
description: nexus 的主要功能综述以及各版本重要变更的追踪记录。
---

# 功能和发布说明

本文档提供了 nexus 主要功能的综合概述，并追踪了各个版本的重要变更。它旨在让开发者清晰了解模板的功能以及它们如何随时间演进。

有关系统架构的详细概述，请参阅[后端开发](/zh/docs/backend)和[前端开发](/zh/docs/frontend)页面。

## 核心功能

nexus 结合了 FastAPI 后端和 React 前端，为构建现代 Web 应用程序提供了完整的解决方案。该模板包括：
...
| Multi-Environment | 支持预演和生产环境 | `docker-compose.*yml` |
| pgAdmin | 数据库管理界面 | `docker-compose.yml` |
| MailCatcher | 本地邮件测试工具 | `docker-compose.yml` |
...
nexus 为使用 Python 和 TypeScript 构建现代 Web 应用程序提供了全面的基础。随着每个版本的发布，模板都在增添新功能、改进安全性以及提高开发体验。
