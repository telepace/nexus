...
const RegisterForm = () => {
  ...
  const handleSubmit = async (event: { preventDefault: () => void }) => {
    ...
    try {
      // Register the user
      const registerResponse = await fetch(`${process.env.PLASMO_PUBLIC_BACKEND_URL}/api/v1/users/`, {
        method: "POST",
        headers: {
      ...
      if (!registerResponse.ok) {
        ...
        throw new Error(registerError.detail || "Registration failed");
      }

      // After successful registration, login the user
      const loginResponse = await fetch(`${process.env.PLASMO_PUBLIC_BACKEND_URL}/api/v1/login/access-token`, {
        method: "POST",
        headers: {
      ...
      if (loginResponse.ok && loginData.access_token) {
        // Store the token
        await storage.set("token", loginData.access_token);
        navigation("/");
      } else {
  ...
  const handleGoogleLogin = async () => {
    try {
      // Similar to login, direct to Google Auth
      chrome.tabs.create({
        ...
        <div className="bg-gray-800/70 backdrop-blur-sm rounded-xl shadow-xl border border-gray-700 p-6">
          <div className="space-y-6">
            ...
            <p className="text-gray-400 text-sm">
              Sign up to use the Nexus extension
            </p>
