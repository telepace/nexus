...
setup("authenticate", async ({ page }) => {
  ...
  try {
    ...
    await expect(page.getByText("Welcome back, nice to see you again!")).toBeVisible({ timeout: 10000 });
    ...
    // 检查登录状态
    try {
      const isErrorVisible = await page.isVisible(
        '.chakra-toast__root[data-type="error"]',
      )
      if (isErrorVisible) {
