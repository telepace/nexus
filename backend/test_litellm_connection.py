    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            req = client.build_request(
                "POST", completion_url, json=payload, headers=headers
            )
            ...
            if response_stream.status_code == 200:
                ...
                async for chunk in response_stream.aiter_bytes():
                    if chunk and chunk_count < 3:
                        print(
                            f"Chunk {chunk_count + 1}: {chunk.decode('utf-8', errors='ignore')[:100]}"
                        )
                        chunk_count += 1
                    if chunk_count >= 3:
                        break
                ...
                print(
                    f"Streaming - Error: {error_text.decode('utf-8', errors='ignore')}"
                )
