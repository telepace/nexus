...
class AIProcessingMonitor:
    ...
    async def analyze_content_item(self, content_item: ContentItem, session: Session):
        ...
        print(f"      Summary: {self._get_status_emoji(status_summary)} {status_summary}")
        print(f"      Key Points: {self._get_status_emoji(status_key_points)} {status_key_points}")
        print(f"      Metadata: {self._get_status_emoji(status_metadata)} {status_metadata}")
        ...
        if status_summary == "failed" or status_key_points == "failed":
            ...
            if ai_result and not ai_result.summary:
                print(f"      Summary问题: 数据库中无summary数据")
            if ai_result and not ai_result.key_points:
                print(f"      Key Points问题: 数据库中无key_points数据")
    ...
    def _analyze_summary_status(self, ai_result: AIResult = None) -> str:
        ...
        if ai_result.summary:
            ...
            if isinstance(ai_result.summary, dict):
                if ai_result.summary.get("text") or ai_result.summary.get("main_thesis"):
                    return "success"
                else:
    ...
    def _analyze_key_points_status(self, ai_result: AIResult = None) -> str:
        ...
        if ai_result.key_points:
            ...
            if isinstance(ai_result.key_points, dict):
                if (ai_result.key_points.get("text") or 
                    ai_result.key_points.get("core_concepts") or
                    ai_result.key_points.get("points")):
                    return "success"
                else:
                    return "empty_content"
    ...
    def _analyze_metadata_status(self, ai_result: AIResult = None) -> str:
        ...
        # 检查是否有基本的元数据
        if (ai_result.labels or 
            ai_result.reading_time_minutes or 
            ai_result.difficulty_level or 
    ...
    def print_current_stats(self):
        ...
        print(f"\n📊 当前统计 (总计: {total})")
        print(f"   Summary成功率: {summary_success_rate:.1f}% ({self.stats['successful_summary']}/{total})")
        print(f"   Key Points成功率: {key_points_success_rate:.1f}% ({self.stats['successful_key_points']}/{total})")
        print(f"   Metadata成功率: {metadata_success_rate:.1f}% ({self.stats['successful_metadata']}/{total})")
    ...
    async def check_specific_timeframe(self, hours_back: int = 24):
        ...
        print(f"📅 时间范围: {start_time.strftime('%Y-%m-%d %H:%M')} - {datetime.utcnow().strftime('%Y-%m-%d %H:%M')}")
        print("=" * 60)
