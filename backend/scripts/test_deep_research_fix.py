...
from app.models.content import ContentItem, Segment, DeepResearchJob
...
async def test_deep_research_segmentation():
    ...
    with Session(engine) as session:
        ...
        statement = (
            select(ContentItem)
            .where(ContentItem.source_uri == f"deep_research:{recent_job.id}")
        )
        ...
        if len(segments) == 0:
            ...
            total_chars = 0
            total_words = 0
            for i, segment in enumerate(segments[:3]):  # 只显示前3个分段
                ...
                print(f"   分段 {segment.segment_index + 1}: {segment.char_count} 字符, {segment.word_count} 词")
...
async def test_api_endpoints():
    ...
    with Session(engine) as session:
        ...
        from app.crud.content import get_content_chunks
