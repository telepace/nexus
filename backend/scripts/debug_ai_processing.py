...
class AIProcessingDebugger:
    ...
    async def debug_content_processing(self, content_id: str = None):
        ...
        print(f"   - Master Key: {'已设置' if settings.LITELLM_MASTER_KEY else '未设置'}")
        print()
...
产生更大的影响。我们需要做好准备，迎接这个AI驱动的未来。
    ...
    async def _test_full_ai_processing(self, content_item: ContentItem, session: Session):
        """测试完整AI处理流程"""
        print("🤖 测试完整AI处理流程...")
    ...
    async def _check_database_storage(self, content_item: ContentItem, session: Session):
        """检查数据库存储"""
        print("💾 检查数据库存储...")
    ...
    async def test_pipeline_processing(self, content_text: str, title: str = "Pipeline测试"):
        """测试预处理管道"""
        print("🔧 测试预处理管道...")
...
async def main():
    ...
    parser.add_argument("--test-connection", action="store_true", help="仅测试LiteLLM连接")
