...
def test_background_task_manager():
    ...
    try:
        ...
        # 测试方法存在
        assert hasattr(manager, "start_ai_regeneration"), (
            "start_ai_regeneration method missing"
        )
        print("✅ start_ai_regeneration method exists")

        assert hasattr(manager, "_regenerate_ai_analysis_async"), (
            "_regenerate_ai_analysis_async method missing"
        )
...
def main():
    ...
    if all(results):
        print(
            "🎉 All tests passed! AI regeneration functionality appears to be working correctly."
        )
        return 0
    else:
