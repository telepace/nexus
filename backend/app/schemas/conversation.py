...
class AIConversationCreate(SQLModel):
    ...
    title: str | None = None
    conversation_type: str = Field(default="user_chat", description="Type of conversation")
    ai_model_name: str = Field(default="gpt-4", description="AI model to use")
    initial_message: str | None = Field(default=None, description="Initial user message")
...
class AnalysisPromptRequest(SQLModel):
    ...
    prompt_type: str = Field(description="Type of analysis: summary, key_points, questions, insights")
    custom_instruction: str | None = Field(default=None, description="Custom instruction to add to the prompt")
...
class ConversationListResponse(SQLModel):
    ...
    total: int
    has_auto_analysis: bool = Field(description="Whether auto analysis conversation exists") 