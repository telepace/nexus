...
class ExtensionStreamRequest(BaseModel):
    """扩展流式请求模型"""
    text: str = Field(..., min_length=1, max_length=50000, description="需要处理的文本内容")
    lang: Literal["auto", "zh", "en"] = Field(default="auto", description="内容语言，auto为自动检测")
    max_tokens: Optional[int] = Field(default=1024, ge=50, le=4096, description="最大生成token数")
...
class ExtensionStreamChunk(BaseModel):
    """流式响应数据块模型"""
    delta: Optional[str] = Field(None, description="增量文本内容")
