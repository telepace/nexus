...
async def _forward_request_to_litellm(
    ...
    try:
        ...
        try:
            ...
            elif "error" in detail_json and "message" in detail_json["error"]: # OpenAI-like error
        ...
        raise HTTPException(status_code=503, detail=f"Error connecting to LLM service: {e}")


@router.post("/completions", response_model=None) # response_model is tricky for streaming
        ...
        if request_data.stream:
            ...
            try:
                response_stream = await _forward_request_to_litellm(
                    client, "POST", litellm_endpoint, data=payload, headers=headers, stream=True
                )
                ...
                if response_stream is not None and hasattr(response_stream, 'aclose') and not response_stream.is_closed:
                    await response_stream.aclose()
        else:
            response_json = await _forward_request_to_litellm(
                client, "POST", litellm_endpoint, data=payload, headers=headers, stream=False
            )
