...
class LiteLLMHealthChecker:
    ...
    async def check_health(self) -> bool:
            ...
            try:
                ...
                if USE_REQUESTS:
                    ...
                    async with httpx.AsyncClient(
                        ...
                        httpx_response = await client.get(health_url, headers=headers)
                        status_code = httpx_response.status_code
                        response_text = httpx_response.text
                ...
                if status_code == 200:
                    ...
                    logger.error(
                        f"LiteLLM 认证失败 - 请检查 LITELLM_MASTER_KEY 配置: {response_text}"
                    )
                    return False  # 认证失败是配置问题，不用尝试其他URL
                elif status_code == 404:
                    logger.warning(
                        f"LiteLLM 健康端点不存在 - 可能服务版本不兼容: {response_text}"
                    )
                    return False  # 404也是服务问题，不用尝试其他URL
                else:
                    logger.warning(
                        f"LiteLLM 健康检查失败 - HTTP {status_code}: {response_text}"
                    )
                    continue  # 尝试下一个URL
                ...
                if USE_REQUESTS and isinstance(e, requests.Timeout):
                    ...
                    not USE_REQUESTS
                    and hasattr(httpx, "TimeoutException")
        ...
        logger.info("诊断建议:")
        logger.info(
            "1. 运行 'curl -H 'Authorization: Bearer sk-telepace' http://localhost:4000/health' 验证服务可用性"
        )
        ...
        try:
            ...
            async with httpx.AsyncClient(
                ...
                response = await client.post(
                    completion_url, json=payload, headers=headers
                )
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"LiteLLM API 功能测试失败 - HTTP {response.status_code}: {response.text}"
                    )
                    return False
    ...
    async def perform_startup_check(self, fail_on_error: bool | None = None) -> bool:
        """执行启动时的全面健康检查
...
async def check_litellm_on_startup(fail_on_error: bool | None = None) -> bool:
    """应用启动时检查 LiteLLM 服务
    ...
    Returns:
        bool: 检查是否通过
    """
    return await litellm_health_checker.perform_startup_check(
