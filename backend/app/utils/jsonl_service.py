...
from .jsonl_parser import (
    JsonlParser, ParseResult, ParsedBlock, ParseOptions, 
    create_parser, ErrorType
)
from .jsonl_formatter import (
    JsonlFormatter, FormatterConfig, format_parse_result,
...
class JsonlService:
    ...
    async def _parse_with_retry(self, content: str, 
                              parse_options: Optional[ParseOptions]) -> ParseResult:
        """带重试的解析"""
        last_exception = None
    ...
    def _generate_cache_key(self, content: str, parse_options: Optional[ParseOptions], 
                          format_type: str) -> str:
        """生成缓存键"""
...
# 便捷函数
def create_service(config: Optional[ServiceConfig] = None) -> JsonlService:
    """创建服务实例"""
