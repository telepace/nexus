...
class ContentChunker:
    ...
    def _detect_chunk_type(self, content: str) -> str:
        ...
        # Check for lists
        if re.match(r'^[\s]*[-*+]\s+', content_stripped, re.MULTILINE) or \
           re.match(r'^[\s]*\d+\.\s+', content_stripped, re.MULTILINE):
            return "list"
        
        # Default to paragraph
        return "paragraph"
    
    def _create_chunk_info(self, content: str, index: int, chunk_type: str) -> ChunkInfo:
    ...
    def create_content_chunks(self, content_item_id: uuid.UUID, content: str) -> List[ContentChunk]:
        """
...
def chunk_content_for_item(content_item_id: uuid.UUID, content: str, 
                          max_chunk_size: int = 3000) -> List[ContentChunk]:
    """
