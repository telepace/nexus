...
def apply_fixes():
    ...
    try:
        ...
        # 修复 deprecated 模块
        try:
            from pydantic.deprecated.class_validators import (
                root_validator as deprecated_root_validator,
            )

            def patched_deprecated_root_validator(
                pre=False, allow_reuse=False, skip_on_failure=None
            ):
                if not pre and skip_on_failure is None:
                    skip_on_failure = True
            ...
            pydantic.deprecated.class_validators.root_validator = (
                patched_deprecated_root_validator
            )
        except ImportError:
            pass
    ...
    try:
        ...
        if hasattr(model_construction, "inspect_namespace"):
            ...
            def patched_inspect_namespace(
                namespace,
                ignored_types=None,
                base_class_vars=None,
                base_class_fields=None,
            ):
                try:
                    return original_inspect_namespace(
                        namespace, ignored_types, base_class_vars, base_class_fields
                    )
