...
from app.models.content import ContentItem, ProcessingJob, ContentAsset
...
class ProcessingResult:
    ...
    assets_created: Optional[List[str]] = None  # List of R2 paths for created assets
...
class JinaProcessor(ProcessingStep):
        ...
        try:
            ...
            if not content_item.title:
                ...
                if not content_item.title:
                    ...
                    content_item.title = f"网页内容 - {parsed_url.hostname}"
    ...
    def _store_markdown_to_r2(self, context: ProcessingContext, markdown_content: str, metadata: Dict[str, Any]) -> str:
        ...
        try:
            ...
            storage_service.upload_file(
                file_data=BytesIO(metadata_bytes),
            ...
            print(f"✅ ContentAsset记录和内容分段创建成功")
...
class MarkItDownProcessor(ProcessingStep):
    ...
    def can_handle(self, content_type: str) -> bool:
        ...
        return content_type in ['url', 'pdf', 'docx', 'text', 'xlsx', 'pptx', 'image', 'audio']
    ...
    def _process_url(self, context: ProcessingContext, result: ProcessingResult) -> ProcessingResult:
        """Process URL content."""
        content_item = context.content_item
    ...
    def _process_text(self, context: ProcessingContext, result: ProcessingResult) -> ProcessingResult:
        ...
        try:
            ...
            result.metadata = {
                ...
                "processor": "markitdown",
                "content_type": "text",
                "word_count": len(content_item.content_text.split()) if content_item.content_text else 0
            }
    ...
    def _process_file(self, context: ProcessingContext, result: ProcessingResult) -> ProcessingResult:
    ...
    def _store_markdown_to_r2(self, context: ProcessingContext, markdown_content: str, metadata: Dict[str, Any]) -> str:
        ...
        try:
            ...
            markdown_asset = ContentAsset(
                ...
                mime_type="text/markdown",
                size_bytes=len(markdown_bytes),
                meta_info=json.dumps({"asset_type": "markdown", "processor": "markitdown"})
            )
            ...
            metadata_asset = ContentAsset(
                ...
                mime_type="application/json",
                size_bytes=len(metadata_bytes),
                meta_info=json.dumps({"asset_type": "metadata", "processor": "markitdown"})
            )
...
class ContentProcessorFactory:
    ...
    @classmethod
    def register_processor(cls, content_type: str, processor_class: Type[ProcessorBase]):
