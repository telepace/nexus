...
async def test_labels_template_with_reading_time():
    ...
    context = {"content": test_content, "content_type": "技术文档"}
...
async def test_labels_template_with_simple_content():
    ...
    context = {"content": simple_content, "content_type": "测试文档"}
...
async def test_labels_template_with_complex_content():
    ...
    context = {"content": complex_content, "content_type": "学术论文"}
    ...
    assert (
        "量子" in " ".join(result["tags"])
        or "计算" in " ".join(result["tags"])
        or "general" in result["tags"]
    )
