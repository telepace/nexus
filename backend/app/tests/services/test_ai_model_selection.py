...
class TestAIModelSelection:
    ...
    async def test_model_selection_logic(self, chat_service):
        ...
        with patch.object(
            chat_service.template_env, "get_template", return_value=mock_template
        ):
            with patch.object(
                chat_service, "_call_litellm_proxy", new_callable=AsyncMock
            ) as mock_call:
                with patch(
                    "app.services.ai.chat_service.tag_manager"
                ) as mock_tag_manager:
                    # 设置mock返回值
                    ...
                    await chat_service.generate_with_template(
                        "labels.j2", {"content": "test"}
                    )
                    mock_call.assert_called_with(
                        "test", "mocked prompt", "deepseek-v3-ensemble"
                    )
    ...
    async def test_different_templates_different_models(self, chat_service):
        ...
        with patch.object(
            ...
            with patch.object(
                chat_service, "_call_litellm_proxy", side_effect=track_calls
            ):
                with patch(
                    "app.services.ai.chat_service.tag_manager"
                ) as mock_tag_manager:
