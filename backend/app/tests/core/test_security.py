...
def mock_settings_key():
    ...
    with patch.object(
        settings, "APP_SYMMETRIC_ENCRYPTION_KEY", TEST_FERNET_KEY_STR
    ) as _fixture:
        yield _fixture
...
def test_decrypt_password_incorrect_key_type():
    ...
    encrypted_password_bytes = cipher_suite_different.encrypt(
