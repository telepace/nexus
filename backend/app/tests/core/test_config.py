...
class TestSettings:
    ...
    def test_sentry_settings(self):
        ...
        assert settings.SENTRY_DSN is None or (
            isinstance(settings.SENTRY_DSN, str)
            or str(settings.SENTRY_DSN).startswith("https://")
        )
    ...
    def test_optional_settings_none_handling(self):
        ...
        for attr in optional_attrs:
            if hasattr(settings, attr):
                ...
                assert (
                    value is None or isinstance(value, str) or hasattr(value, "__str__")
                )
            ...
            if hasattr(settings, attr):
                ...
                if attr_value and len(str(attr_value)) > 10:
                    ...
                    if attr_str in settings_str:
                        ...
                        logger.warning(
                            f"Sensitive attribute {attr} appears in settings string representation"
                        )
                    # 对于测试环境，我们可以放宽这个要求
