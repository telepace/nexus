...
from app.crud import crud_content # Assuming sync create_content_item_sync is aliased or directly used
...
def create_random_content_item(db: Session, *, user_id: uuid.UUID) -> ContentItem:
    ...
    created_item = crud_content.create_content_item_sync(session=db, content_item_in=item_data)
    return created_item
