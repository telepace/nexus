...
class MockStorage(StorageInterface):
    async def upload_file(self, file_content: bytes, destination_blob_name: str, content_type: str | None = None) -> str:
...
def test_extract_images_from_pdf_success(mock_fitz):
    ...
    mock_page.get_images.return_value = [(7, 0, 100, 100, 8, "DeviceRGB", "", "Image1", "", 0)] 
...
def test_extract_images_from_pdf_no_fitz():
    ...
    with patch("app.utils.image_processor.PYMUPDF_AVAILABLE", False):
        with pytest.raises(RuntimeError, match="PyMuPDF \(fitz\) is not installed"):
...
async def test_process_web_image_keep_link(mock_storage_service):
    ...
    result = await process_web_image(image_url, strategy="keep_link", storage=mock_storage_service)
...
async def test_process_web_image_download_success(mock_async_client, mock_storage_service):
    ...
    mock_storage_service.upload_file = AsyncMock(return_value=f"mock_public_url/user_images/{user_id}/some_uuid.jpg")
...
@pytest.mark.asyncio
@patch("app.utils.image_processor.httpx.AsyncClient")
async def test_process_web_image_download_failure(mock_async_client, mock_storage_service):
    image_url = "http://example.com/nonexistent.png"
    user_id = uuid.uuid4()

    mock_async_client.return_value.__aenter__.return_value.get.side_effect = Exception("Download failed")
...
async def test_process_base64_image_success(mock_storage_service):
    ...
    mock_storage_service.upload_file = AsyncMock(return_value=f"mock_public_url/user_images/{user_id}/some_uuid.png")
...
def test_assess_image_importance():
    ...
    assert assess_image_importance(image_size=5 * 1024) == "medium" # 5KB - based on current logic
...
async def test_check_image_accessibility_unreachable(mock_async_client):
    ...
    mock_async_client.return_value.__aenter__.return_value.head.side_effect = Exception("HTTP Error")
