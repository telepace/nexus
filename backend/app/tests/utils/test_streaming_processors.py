...
class TestStreamingAIProcessor:
    ...
    async def test_process_streaming_success(self, processor, content_item):
        ...
        with patch.object(processor, '_render_template', return_value="test prompt") as mock_render, \
            ...
            chunks = []
            async for chunk in processor.process_streaming(content_item, "summary", mock_session):
                chunks.append(chunk)
    ...
    async def test_process_streaming_with_error_recovery(self, processor, content_item):
        ...
        with patch.object(processor, '_render_template', side_effect=Exception("Template error")):
            chunks = []
            async for chunk in processor.process_streaming(content_item, "summary", mock_session):
                chunks.append(chunk)
    ...
    async def test_stream_llm_call_http_error(self, processor):
        """测试LLM调用HTTP错误"""
        with patch('app.utils.streaming_processors.httpx.AsyncClient') as mock_client_class:
    ...
    async def test_stream_llm_call_with_mock_response(self, processor):
        ...
        async def mock_stream_call(system_content, user_prompt):
            # 模拟正常的流式响应
...
class TestStreamingSummaryProcessor:
    ...
    async def test_generate_summary_stream(self, processor, content_item):
        ...
        with patch.object(processor, 'process_streaming') as mock_process:
            ...
            result = []
            async for chunk in processor.generate_summary_stream(content_item, mock_session):
                result.append(chunk)
...
class TestStreamingKeyPointsProcessor:
    ...
    async def test_generate_key_points_stream(self, processor, content_item):
        ...
        with patch.object(processor, 'process_streaming') as mock_process:
            ...
            result = []
            async for chunk in processor.generate_key_points_stream(content_item, mock_session):
                result.append(chunk)
            
            assert len(result) > 0
            mock_process.assert_called_once_with(content_item, "key_points", mock_session)
...
class TestStreamingProcessorsIntegration:
    ...
    async def test_full_processing_flow(self, content_item):
        ...
        with patch.object(summary_processor, '_render_template', return_value="摘要提示"):
            with patch.object(summary_processor, '_stream_llm_call') as mock_stream:
                ...
                chunks = []
                async for chunk in summary_processor.process_streaming(content_item, "summary", mock_session):
                    chunks.append(chunk)
    ...
    async def test_error_handling_in_flow(self, content_item):
        ...
        with patch.object(processor, '_render_template', side_effect=Exception("渲染失败")):
            chunks = []
            async for chunk in processor.process_streaming(content_item, "summary", mock_session):
                chunks.append(chunk)
    ...
    async def test_multiple_processors_parallel(self, content_item):
        ...
        # 分别模拟两个处理器的行为
        with patch.object(summary_processor, 'process_streaming') as mock_summary_process:
            with patch.object(keypoints_processor, 'process_streaming') as mock_keypoints_process:
                ...
                async for chunk in summary_processor.generate_summary_stream(content_item, mock_session):
                    summary_result.append(chunk)
                
                async for chunk in keypoints_processor.generate_key_points_stream(content_item, mock_session):
