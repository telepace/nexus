...
from app.utils.text_segmentation import (
...
内容二"""
        ...
        with patch.object(service, '_segment_by_sections') as mock_sections:
            # 确保分段内容足够长，避免被优化算法合并
            long_content_1 = "分段1内容" + "这是一个足够长的内容段落，用于测试语义分段功能。" * 20
            long_content_2 = "分段2内容" + "这是另一个足够长的内容段落，用于测试语义分段功能。" * 20
            
            mock_sections.return_value = [
                {"content": long_content_1, "order": 1, "type": "section", "word_count": len(long_content_1.split()), "char_count": len(long_content_1), "summary": "分段1内容", "metadata": {"segmentation_method": "section", "is_complete": True}, "id": "segment_1"},
                {"content": long_content_2, "order": 2, "type": "section", "word_count": len(long_content_2.split()), "char_count": len(long_content_2), "summary": "分段2内容", "metadata": {"segmentation_method": "section", "is_complete": True}, "id": "segment_2"}
            ]
...
段落三内容。"""
        ...
        with patch.object(service, '_segment_by_sections') as mock_sections, \
            ...
            # 章节分段只返回一个分段（效果不好）
            mock_sections.return_value = [{"content": content, "order": 1, "type": "section", "word_count": len(content.split()), "char_count": len(content), "summary": "段落一内容", "metadata": {"segmentation_method": "section", "is_complete": True}, "id": "segment_1"}]
            
            # 确保段落分段返回的内容足够长
            long_paragraph_1 = "段落1内容" + "这是第一个足够长的段落内容，用于测试段落分段功能。" * 20
            long_paragraph_2 = "段落2内容" + "这是第二个足够长的段落内容，用于测试段落分段功能。" * 20
            
            mock_paragraphs.return_value = [
                {"content": long_paragraph_1, "order": 1, "type": "paragraph", "word_count": len(long_paragraph_1.split()), "char_count": len(long_paragraph_1), "summary": "段落1内容", "metadata": {"segmentation_method": "paragraph", "is_complete": True}, "id": "segment_1"},
                {"content": long_paragraph_2, "order": 2, "type": "paragraph", "word_count": len(long_paragraph_2.split()), "char_count": len(long_paragraph_2), "summary": "段落2内容", "metadata": {"segmentation_method": "paragraph", "is_complete": True}, "id": "segment_2"}
            ]
    ...
    async def test_segment_by_length(self, service):
        ...
        # 验证每个分段的长度不超过最大限制
        for segment in result:
            assert len(segment["content"]) <= service.max_segment_length + service.overlap_length
    ...
    async def test_segment_content_error_handling(self, service):
        ...
        # 模拟分段过程中的错误
        with patch.object(service, '_segment_by_sections', side_effect=Exception("Test error")):
            result = await service.segment_content(
