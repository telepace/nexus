...
@patch(
    "app.models.Tag"
)  # Patch Tag in app.models namespace since it's imported dynamically
def test_create_tag(MockTagInModels: MagicMock, mock_db_session: MagicMock):
    ...
    MockTagInModels.return_value = mock_tag_instance  # crud.Tag(**data) returns this

    created_tag = crud.create_tag(db=mock_db_session, tag_in=tag_in_schema)

    MockTagInModels.assert_called_once_with(**tag_in_schema.model_dump())
...
def test_create_tag_integrity_error(
    MockTagInModels: MagicMock, mock_db_session: MagicMock
):
    tag_in_data = {"name": "Test Tag Fail"}
    tag_in_schema = MockTagCreateSchema(**tag_in_data)
    MockTagInModels.return_value = MagicMock()
    mock_db_session.commit.side_effect = IntegrityError(
        "mock_commit_tag", {}, Exception()
    )

    with pytest.raises(IntegrityError):
        crud.create_tag(db=mock_db_session, tag_in=tag_in_schema)

    mock_db_session.rollback.assert_not_called()  # crud.create_tag does not handle rollback


@patch("app.models.Tag")
def test_get_tag(MockTagInModels: MagicMock, mock_db_session: MagicMock):
    tag_id = uuid.uuid4()
    mock_tag_instance = MagicMock()
    mock_db_session.get.return_value = mock_tag_instance
    tag = crud.get_tag(db=mock_db_session, tag_id=tag_id)
    mock_db_session.get.assert_called_once_with(MockTagInModels, tag_id)
    assert tag == mock_tag_instance


@patch("app.models.Tag")
def test_get_tag_not_found(MockTagInModels: MagicMock, mock_db_session: MagicMock):
    tag_id = uuid.uuid4()
...
def test_get_tags_no_items(
    mock_select_crud: MagicMock, MockTagInModels: MagicMock, mock_db_session: MagicMock
):
    ...
    assert tags == []
    mock_select_crud.assert_called_once_with(MockTagInModels)
...
@patch("app.models.Tag")
@patch("app.crud.select")
def test_get_tags_pagination_skip(
    mock_select_crud: MagicMock, MockTagInModels: MagicMock, mock_db_session: MagicMock
):
    mock_query_obj = MagicMock()
    mock_select_crud.return_value = mock_query_obj
...
@patch("app.models.Tag")
@patch("app.crud.select")
def test_get_tags_pagination_limit(
    mock_select_crud: MagicMock, MockTagInModels: MagicMock, mock_db_session: MagicMock
):
    mock_query_obj = MagicMock()
    mock_select_crud.return_value = mock_query_obj
...
def test_update_tag_success(mock_db_session: MagicMock):
    ...
    updated_tag = crud.update_tag(
        db=mock_db_session, tag=mock_existing_tag, tag_in=tag_update_schema
    )
    assert mock_existing_tag.name == "Updated Tag Name"
    assert mock_existing_tag.description == "Updated Desc"
    mock_db_session.add.assert_called_with(mock_existing_tag)
    mock_db_session.commit.assert_called_once()
    mock_db_session.refresh.assert_called_with(mock_existing_tag)
...
def test_update_tag_integrity_error(mock_db_session: MagicMock):
    ...
    tag_update_schema = MockTagUpdateSchema(**update_data_dict)
    mock_db_session.commit.side_effect = IntegrityError(
        "mock_update_tag_fail", {}, Exception()
    )

    with pytest.raises(
        IntegrityError
    ):  # crud.update_tag does not handle IntegrityError
        crud.update_tag(
            db=mock_db_session, tag=mock_existing_tag, tag_in=tag_update_schema
        )
