...
def test_generic_update_found_and_success(mock_db_session: MagicMock):
    ...
    mock_db_session.get.assert_called_once_with(MockDbModel, obj_id)
    assert mock_existing_obj.name == "Updated Name"; assert mock_existing_obj.value == 101
    mock_db_session.add.assert_called_once_with(mock_existing_obj)
    mock_db_session.commit.assert_called_once(); mock_db_session.refresh.assert_called_once_with(mock_existing_obj)
...
def test_generic_update_partial(mock_db_session: MagicMock):
    ...
    updated_obj = crud.update(session=mock_db_session, model=MockDbModel, id=obj_id, obj_in=update_schema)
    assert mock_existing_obj.name == "Updated Name Only"; assert mock_existing_obj.value == 100 
    mock_db_session.add.assert_called_once_with(mock_existing_obj); assert updated_obj == mock_existing_obj

def test_generic_update_not_found(mock_db_session: MagicMock):
    obj_id = uuid.uuid4(); mock_db_session.get.return_value = None
    update_schema = MockUpdateSchema(name="Any Update")
    updated_obj = crud.update(session=mock_db_session, model=MockDbModel, id=obj_id, obj_in=update_schema)
    mock_db_session.get.assert_called_once_with(MockDbModel, obj_id)
    mock_db_session.add.assert_not_called(); mock_db_session.commit.assert_not_called()
    assert updated_obj is None

def test_generic_update_integrity_error(mock_db_session: MagicMock):
    obj_id = uuid.uuid4(); mock_existing_obj = MockDbModel(id=obj_id, name="Old Name")
    mock_db_session.get.return_value = mock_existing_obj
...
def test_generic_delete_found(mock_db_session: MagicMock):
    obj_id = uuid.uuid4(); mock_instance = MockDbModel(id=obj_id)
    mock_db_session.get.return_value = mock_instance
    ...
    mock_db_session.commit.assert_called_once(); assert deleted_obj == mock_instance

def test_generic_delete_not_found(mock_db_session: MagicMock):
    obj_id = uuid.uuid4(); mock_db_session.get.return_value = None
    deleted_obj = crud.delete(session=mock_db_session, model=MockDbModel, id=obj_id)
    mock_db_session.get.assert_called_once_with(MockDbModel, obj_id)
    mock_db_session.delete.assert_not_called(); mock_db_session.commit.assert_not_called()
