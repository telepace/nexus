...
def test_get_content_share_by_token(db: Session) -> None:
    ...
    retrieved_share = crud_content.get_content_share_by_token(
        db, token=created_share.share_token
    )
    ...
    retrieved_share_none = crud_content.get_content_share_by_token(
        db, token=non_existent_token
    )
    ...
    retrieved_inactive_share = crud_content.get_content_share_by_token(
        db, token=created_share.share_token
    )
...
def test_get_content_shares_by_content_id(db: Session) -> None:
    ...
    shares_for_ci1 = crud_content.get_content_shares_by_content_id(
        db, content_item_id=content_item1.id
    )
    assert len(shares_for_ci1) == 1
    assert shares_for_ci1[0].id == share1_ci1.id
    assert shares_for_ci1[0].is_active is True

    shares_for_ci2 = crud_content.get_content_shares_by_content_id(
        db, content_item_id=content_item2.id
    )
    assert len(shares_for_ci2) == 1
    assert shares_for_ci2[0].id == share1_ci2.id

    non_existent_uuid = uuid.uuid4()
    shares_for_none = crud_content.get_content_shares_by_content_id(
        db, content_item_id=non_existent_uuid
    )
...
def test_increment_access_count(db: Session) -> None:
    ...
    share = crud_content.increment_access_count(
        db, content_share=share
    )  # Reaches max_access_count
...
def test_update_content_share(db: Session) -> None:
    ...
    updated_share = crud_content.update_content_share(
    ...
    reactivated_share = crud_content.update_content_share(
        db, content_share=updated_share, update_data=update_data_reactivate
    )
    assert reactivated_share.is_active is True
    assert (
        reactivated_share.password_hash == current_password_hash
    )  # Ensure hash did not change


print(
    "CRUD tests for ContentShare created in backend/app/tests/crud/test_crud_share.py"
)
