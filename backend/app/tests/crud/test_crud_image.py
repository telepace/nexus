...
from sqlalchemy.future import select # Ensure this is the correct select for async
...
# --- Test Data ---
def create_test_image_model(owner_id: uuid.UUID, image_id: uuid.UUID = None, s3_key: str = "test.jpg") -> Image:
    return Image(
...
async def test_get_image(mock_db_session: AsyncMock):
    ...
    mock_db_session.execute.return_value.scalar_one_or_none = MagicMock(return_value=expected_image)
...
async def test_get_multi_images_by_owner(mock_db_session: AsyncMock):
    ...
    mock_db_session.execute.return_value.scalars.return_value.all = MagicMock(return_value=expected_images)
...
async def test_update_image(mock_db_session: AsyncMock):
    ...
    db_image_to_update = create_test_image_model(owner_id=owner_id, image_id=image_id, s3_key="old.jpg")
...
async def test_remove_image(mock_db_session: AsyncMock):
    ...
    mock_db_session.execute.return_value.scalar_one_or_none = MagicMock(return_value=image_to_delete)
