...
class TestAIRegeneration:
    ...
    def test_regenerate_ai_analysis_success(self, mock_content_item, mock_current_user, mock_session):
        """测试成功的AI重新生成"""
        # 设置模拟
        ...
        with patch('app.api.routes.content.background_task_manager') as mock_btm:
            ...
            mock_btm.start_ai_regeneration.assert_called_once_with(
                content_id=str(mock_content_item.id),
    ...
    def test_regenerate_ai_analysis_content_not_found(self, mock_current_user, mock_session):
        """测试内容不存在的情况"""
        # 设置模拟 - 内容不存在
    ...
    def test_regenerate_ai_analysis_permission_denied(self, mock_content_item, mock_current_user, mock_session):
        """测试权限不足的情况"""
        # 设置模拟 - 不同的用户ID
        mock_session.get.return_value = mock_content_item
    ...
    def test_regenerate_ai_analysis_no_content_text(self, mock_content_item, mock_current_user, mock_session):
        """测试没有内容文本的情况"""
        # 设置模拟 - 没有内容文本
        ...
        assert "400" in str(exc_info.value) or "must be processed" in str(exc_info.value).lower()
...
class TestBackgroundTaskManager:
    ...
    async def test_regenerate_ai_analysis_async_success(self):
        ...
        with patch('app.utils.background_tasks.Session') as mock_session_class, \
            ...
            mock_pipeline._ai_initialization_layer = AsyncMock(return_value=(ai_results, ai_stats))
            ...
            # 验证调用了正确的方法
            mock_pipeline._ai_initialization_layer.assert_called_once()
    ...
    async def test_regenerate_ai_analysis_async_no_content_text(self):
        ...
        with patch('app.utils.background_tasks.Session') as mock_session_class, \
            ...
            assert "no processed text" in str(exc_info.value).lower()
