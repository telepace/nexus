...
class TestTimezoneHTTPMiddleware:
    ...
    async def test_dispatch_with_timezone_header(self, middleware, mock_request):
        ...
        with patch.object(
            middleware, "_process_response_timezone", return_value=mock_response
        ) as mock_process:
            _response = await middleware.dispatch(mock_request, call_next)
    ...
    def test_extract_timezone_from_request_priority(self, middleware, mock_request):
        """测试时区提取的优先级 - 头部优先"""
        mock_request.headers = {"x-user-timezone": "Asia/Shanghai"}
        mock_request.query_params = {"timezone": "America/New_York"}

        with patch(
            "app.api.middlewares.timezone_middleware.TimezoneUtil.is_valid_timezone",
            return_value=True,
        ):
    ...
    def test_extract_timezone_from_request_invalid_timezone(
    ...
    def test_extract_timezone_from_request_query_fallback(
        self, middleware, mock_request
    ):
        """测试查询参数回退"""
    ...
    async def test_process_response_timezone_success(self, middleware):
        ...
        with patch(
            "app.api.middlewares.timezone_middleware.TimezoneMiddleware.add_timezone_to_response",
            return_value={"created_at": "2023-01-01T08:00:00+08:00"},
        ) as mock_add_tz:
...
class TestTimezoneResponseHelper:
    ...
    def test_format_datetime_fields_without_timezone(self):
        ...
        with patch(
            ...
            _result = TimezoneResponseHelper.format_datetime_fields(test_data, None)
    ...
    def test_create_timezone_aware_response_dict(self):
        ...
        with patch.object(
            TimezoneResponseHelper, "format_datetime_fields", return_value=test_data
        ) as mock_format:
