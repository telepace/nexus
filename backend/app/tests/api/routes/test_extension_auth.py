...
def test_check_status_auth_header_valid_user(
    ...
    app_for_extension_auth.dependency_overrides[get_db] = (
    ...
    mock_jwt.decode.assert_called_once_with(
        "validtoken", settings.SECRET_KEY, algorithms=[security.ALGORITHM]
    )
...
def test_check_status_cookie_valid_user(
    ...
    mock_user_public_dict = EXAMPLE_USER_PUBLIC_DICT.copy()
    mock_user_public_dict.update(
        {"id": user_id, "email": "cookieuser@example.com", "full_name": "Cookie User"}
    )
    MockUserPublicModel.model_validate.return_value = mock_user_public_dict
    ...
    assert data["user"] == mock_user_public_dict
    mock_jwt.decode.assert_called_once_with(
        "validcookietoken", settings.SECRET_KEY, algorithms=[security.ALGORITHM]
    )
    mock_db_session_fixture.get.assert_called_once_with(MockUserModel, user_id)


@patch("app.api.deps.get_db")
def test_check_status_no_token(
    mock_get_db_patch: MagicMock,
    app_for_extension_auth: FastAPI,
    mock_db_session_fixture: MagicMock,
):
...
@patch("app.api.deps.get_db")
@patch("app.api.routes.extension_auth.jwt")
def test_check_status_invalid_header_token(
    mock_jwt: MagicMock,
    mock_get_db_patch: MagicMock,
    app_for_extension_auth: FastAPI,
    mock_db_session_fixture: MagicMock,
...
@patch("app.api.deps.get_db")
@patch("app.api.routes.extension_auth.jwt")
def test_check_status_invalid_cookie_token(
    mock_jwt: MagicMock,
    mock_get_db_patch: MagicMock,
    app_for_extension_auth: FastAPI,
    mock_db_session_fixture: MagicMock,
...
@patch("app.api.deps.get_db")
@patch("app.api.routes.extension_auth.jwt")
def test_check_status_user_not_found_header(
    mock_jwt: MagicMock,
    mock_get_db_patch: MagicMock,
    MockUserModel: MagicMock,
    app_for_extension_auth: FastAPI,
...
@patch("app.api.deps.get_db")
@patch("app.api.routes.extension_auth.jwt")
def test_check_status_user_inactive_header(
    mock_jwt: MagicMock,
    mock_get_db_patch: MagicMock,
    MockUserModel: MagicMock,
    app_for_extension_auth: FastAPI,
...
@patch("app.api.deps.get_db")
@patch("app.api.routes.extension_auth.jwt")
def test_check_status_general_exception_on_decode(
    mock_jwt: MagicMock,
    mock_get_db_patch: MagicMock,
    app_for_extension_auth: FastAPI,
    mock_db_session_fixture: MagicMock,
...
def test_get_extension_token_success(
    ...
    mock_jwt.decode.assert_called_once_with(
        "validwebcookietoken", settings.SECRET_KEY, algorithms=[security.ALGORITHM]
    )
    mock_db_session_fixture.get.assert_called_once_with(MockUserModel, user_id)
...
def test_get_extension_token_no_cookie(
    app_for_extension_auth: FastAPI, mock_db_session_fixture: MagicMock
):
    """Test getting extension token without cookie"""
    app_for_extension_auth.dependency_overrides[get_db] = (
        lambda: mock_db_session_fixture
    )
    with TestClient(app_for_extension_auth) as client:
    ...
    assert "未找到有效的网页会话" in error_detail


def test_get_extension_token_invalid_cookie(
    app_for_extension_auth: FastAPI, mock_db_session_fixture: MagicMock, mocker
):
    """Test getting extension token with invalid cookie"""
    # Mock jwt.decode to raise an exception
    mocker.patch(
        "app.api.routes.extension_auth.jwt.decode",
        side_effect=Exception("Cookie token is bad"),
    )
    ...
    assert response.status_code == 401
    error_detail = get_error_detail(response.json())
    assert "获取扩展令牌失败: Cookie token is bad" in error_detail


def test_get_extension_token_user_not_found(
    app_for_extension_auth: FastAPI, mock_db_session_fixture: MagicMock, mocker
):
    """Test getting extension token when user not found"""
    ...
    assert response.status_code == 401
    error_detail = get_error_detail(response.json())
    assert "获取扩展令牌失败" in error_detail


def test_get_extension_token_user_inactive(
    app_for_extension_auth: FastAPI, mock_db_session_fixture: MagicMock, mocker
):
    """Test getting extension token when user is inactive"""
    user_id = uuid.uuid4()

    # Mock jwt.decode to return a valid payload
    mocker.patch(
        "app.api.routes.extension_auth.jwt.decode", return_value={"sub": str(user_id)}
    )

    # Mock session.get to return an inactive user
    mock_user = MagicMock()
    mock_user.is_active = False
    mock_db_session_fixture.get.return_value = mock_user
...
def test_get_extension_token_create_token_exception(
    ...
    """Test getting extension token when token creation fails"""
    user_id = uuid.uuid4()
    ...
    # Mock session.get to return an active user
    mock_user = MagicMock()
    ...
    mocker.patch(
        "app.api.routes.extension_auth.security.create_access_token",
    ...
    assert response.status_code == 401
    error_detail = get_error_detail(response.json())
    assert "获取扩展令牌失败: Token creation failed" in error_detail
