...
from app.schemas.image import ImageResponse, PresignedURLResponse, ImageCreate
...
def test_user() -> UserModel:
    return UserModel(id=uuid.uuid4(), email="test@example.com", is_active=True, is_superuser=False)
...
async def test_get_upload_url_success(
    ...
    mock_storage_instance.get_presigned_url = AsyncMock(return_value="http://s3.mock/presigned-url-for-upload")
    ...
    assert call_args['blob_name'] == data["s3_key"] # s3_key is generated, so check against returned one
...
async def test_create_image_record_success(
    ...
    assert data["s3_key"] == image_response_dict["s3_key"] # s3_key from model, not payload
...
async def test_read_image_success(
    ...
    mock_get_image.assert_called_once_with(db=unittest.mock.ANY, image_id=test_image_model.id)
...
async def test_read_image_forbidden(
    ...
    mock_get_current_user: MagicMock,
    client: AsyncClient,
    test_user: UserModel, # This is the current user
...
async def test_delete_image_success(
    ...
    mock_get_current_user.return_value = test_user
    mock_get_image.return_value = test_image_model
    mock_remove_image.return_value = test_image_model # remove_image returns the deleted obj
    ...
    mock_get_image.assert_called_once_with(db=unittest.mock.ANY, image_id=test_image_model.id)
    mock_storage_instance.delete_file.assert_called_once_with(blob_name=test_image_model.s3_key)
    mock_remove_image.assert_called_once_with(db=unittest.mock.ANY, image_id=test_image_model.id)
