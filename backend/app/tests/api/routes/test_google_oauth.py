...
def client(app_for_google_oauth_module: FastAPI, mock_db_session_fixture: MagicMock):
    app_for_google_oauth_module.dependency_overrides[get_db] = lambda: mock_db_session_fixture
...
def test_google_login_success(mock_starlette_session: MagicMock, mock_token_urlsafe: MagicMock, mock_route_settings: MagicMock, client: TestClient):
    ...
    mock_starlette_session.__setitem__.assert_any_call("google_oauth_state", "test_state_token")

@patch("app.api.routes.google_oauth.settings")
@patch("secrets.token_urlsafe", return_value="test_state_token_ext")
@patch("starlette.requests.Request.session", new_callable=MagicMock)
def test_google_login_success_with_extension_callback(mock_starlette_session: MagicMock, mock_token_urlsafe: MagicMock, mock_route_settings: MagicMock, client: TestClient):
    ...
    assert response.status_code == 307
    mock_starlette_session.__setitem__.assert_any_call("google_oauth_state", "test_state_token_ext")
...
def test_google_callback_success_new_user(
    ...
    mock_route_settings.GOOGLE_CLIENT_ID = "test_cid"; mock_route_settings.GOOGLE_CLIENT_SECRET = "test_csec"; mock_route_settings.google_oauth_redirect_uri = "http://cb.uri"

    mock_token_response = MagicMock(); mock_token_response.json.return_value = {"access_token": "google_access_token", "id_token": "mock_id_token"}
    ...
    mock_session_obj = MagicMock(); session_items = {"google_oauth_state": "test_state_value"}
    mock_session_obj.get = MagicMock(side_effect=session_items.get); mock_session_obj.pop = MagicMock(side_effect=lambda k,d=None: session_items.pop(k,d))
    with patch("starlette.requests.Request.session", mock_session_obj):
        response = client.get(
            f"{app_settings.API_V1_STR}/login/google/callback?code=auth_code_here&state=test_state_value", follow_redirects=False
        )
    assert response.status_code == 307
    assert response.headers["location"] == f"{app_settings.FRONTEND_HOST}/login/google/callback?token={new_app_token}"
...
def test_google_callback_success_existing_user_no_google_id(
    ...
    mock_token_response = MagicMock(); mock_token_response.json.return_value = {"access_token": "ga_token"}
    mock_user_info_resp = MagicMock(); google_user_data = EXAMPLE_GOOGLE_USER_INFO.copy(); mock_user_info_resp.json.return_value = google_user_data
    mock_requests.post.return_value = mock_token_response; mock_requests.get.return_value = mock_user_info_resp
    existing_user = MagicMock(id="existing_user_456", email=google_user_data["email"], google_id=None, is_active=True)
    mock_crud.get_user_by_email.return_value = existing_user
    new_app_token = "app_token_existing_user_no_gid"; mock_create_access_token.return_value = new_app_token
    mock_session_obj = MagicMock(); session_items = {"google_oauth_state": "test_state_value"}
    mock_session_obj.get = MagicMock(side_effect=session_items.get); mock_session_obj.pop = MagicMock(side_effect=lambda k,d=None: session_items.pop(k,d))
    with patch("starlette.requests.Request.session", mock_session_obj):
        response = client.get(f"{app_settings.API_V1_STR}/login/google/callback?code=auth_code&state=test_state_value", follow_redirects=False)
    assert response.status_code == 307; assert response.headers["location"] == f"{app_settings.FRONTEND_HOST}/login/google/callback?token={new_app_token}"
    mock_crud.create_user_oauth.assert_not_called(); mock_db_session_fixture.add.assert_called_with(existing_user); assert existing_user.google_id == google_user_data["sub"]
...
def test_google_callback_success_existing_user_with_google_id(
    ...
    mock_user_info_resp = MagicMock(); google_user_data = EXAMPLE_GOOGLE_USER_INFO.copy(); mock_user_info_resp.json.return_value = google_user_data
    mock_requests.post.return_value = mock_token_response; mock_requests.get.return_value = mock_user_info_resp
    existing_user = MagicMock(id="existing_user_789", email=google_user_data["email"], google_id=google_user_data["sub"], is_active=True)
    mock_crud.get_user_by_email.return_value = existing_user
    new_app_token = "app_token_existing_user_with_gid"; mock_create_access_token.return_value = new_app_token
    mock_session_obj = MagicMock(); session_items = {"google_oauth_state": "test_state_value"}
    mock_session_obj.get = MagicMock(side_effect=session_items.get); mock_session_obj.pop = MagicMock(side_effect=lambda k,d=None: session_items.pop(k,d))
    with patch("starlette.requests.Request.session", mock_session_obj):
        response = client.get(f"{app_settings.API_V1_STR}/login/google/callback?code=auth_code&state=test_state_value", follow_redirects=False)
    assert response.status_code == 307; assert response.headers["location"] == f"{app_settings.FRONTEND_HOST}/login/google/callback?token={new_app_token}"
    mock_crud.create_user_oauth.assert_not_called(); mock_db_session_fixture.add.assert_not_called()
...
def test_google_callback_with_extension_redirect(
    ...
    mock_route_settings.configure_mock(**app_settings.model_dump(exclude_none=True)); mock_route_settings.GOOGLE_CLIENT_ID = "test_cid"; mock_route_settings.GOOGLE_CLIENT_SECRET = "test_csec"; mock_route_settings.google_oauth_redirect_uri = "http://cb.uri"
    mock_token_response = MagicMock(); mock_token_response.json.return_value = {"access_token": "ga_token"}
    mock_user_info_resp = MagicMock(); mock_user_info_resp.json.return_value = EXAMPLE_GOOGLE_USER_INFO.copy()
    mock_requests.post.return_value = mock_token_response; mock_requests.get.return_value = mock_user_info_resp
    new_user_instance = MagicMock(id="ext_cb_user"); mock_crud.get_user_by_email.return_value = None
    MockUserModel.return_value = new_user_instance; mock_crud.create_user_oauth.return_value = new_user_instance
    new_app_token = "app_token_ext_cb"; mock_create_access_token.return_value = new_app_token
    mock_session_obj = MagicMock()
    session_items = {"google_oauth_state": "test_state_ext_cb", "extension_callback": "myextension://callback"}
    mock_session_obj.get = MagicMock(side_effect=session_items.get); mock_session_obj.pop = MagicMock(side_effect=lambda k,d=None: session_items.pop(k,d)) # pop will modify session_items
    with patch("starlette.requests.Request.session", mock_session_obj):
        response = client.get(f"{app_settings.API_V1_STR}/login/google/callback?code=auth_code_ext&state=test_state_ext_cb", follow_redirects=False)
    assert response.status_code == 307; assert response.headers["location"] == f"myextension://callback?token={new_app_token}"
    assert "extension_callback" not in session_items

@patch("app.api.routes.google_oauth.settings")
def test_google_callback_error_from_google(mock_route_settings: MagicMock, client: TestClient):
    mock_route_settings.FRONTEND_HOST = app_settings.FRONTEND_HOST
    mock_session_obj = MagicMock(); session_items = {"google_oauth_state": "test_state_value"}
    mock_session_obj.get = MagicMock(side_effect=session_items.get); mock_session_obj.pop = MagicMock(side_effect=lambda k,d=None: session_items.pop(k,d))
    with patch("starlette.requests.Request.session", mock_session_obj):
        response = client.get(f"{app_settings.API_V1_STR}/login/google/callback?error=access_denied&state=test_state_value", follow_redirects=False)
    assert response.status_code == 307; assert response.headers["location"] == f"{app_settings.FRONTEND_HOST}/login?error=oauth_error&message=access_denied"

@patch("app.api.routes.google_oauth.settings")
def test_google_callback_state_mismatch(mock_route_settings: MagicMock, client: TestClient):
    mock_route_settings.FRONTEND_HOST = app_settings.FRONTEND_HOST
    mock_session_obj = MagicMock(); mock_session_obj.get.return_value = "stored_correct_state"; mock_session_obj.pop.return_value = "stored_correct_state"
    with patch("starlette.requests.Request.session", mock_session_obj):
        response = client.get(f"{app_settings.API_V1_STR}/login/google/callback?code=somecode&state=received_wrong_state", follow_redirects=False)
    assert response.status_code == 307; assert response.headers["location"] == f"{app_settings.FRONTEND_HOST}/login?error=invalid_state"
...
def test_google_callback_no_code(mock_route_settings: MagicMock, client: TestClient):
    ...
    mock_session_obj.get = MagicMock(side_effect=session_items.get); mock_session_obj.pop = MagicMock(side_effect=lambda k,d=None: session_items.pop(k,d))
    with patch("starlette.requests.Request.session", mock_session_obj):
        response = client.get(f"{app_settings.API_V1_STR}/login/google/callback?state=test_state_value", follow_redirects=False)
    assert response.status_code == 307; assert response.headers["location"] == f"{app_settings.FRONTEND_HOST}/login?error=no_code"

@patch("app.api.routes.google_oauth.settings")
@patch("app.api.routes.google_oauth.requests")
def test_google_callback_token_exchange_fails(mock_requests: MagicMock, mock_route_settings: MagicMock, client: TestClient):
    mock_route_settings.configure_mock(**app_settings.model_dump(exclude_none=True)); mock_route_settings.GOOGLE_CLIENT_ID = "test_cid"; mock_route_settings.GOOGLE_CLIENT_SECRET = "test_csec"; mock_route_settings.google_oauth_redirect_uri = "http://cb.uri"
    mock_requests.post.side_effect = requests.exceptions.HTTPError("Failed to get token")
    mock_session_obj = MagicMock(); session_items = {"google_oauth_state": "test_state_value"}
    mock_session_obj.get = MagicMock(side_effect=session_items.get); mock_session_obj.pop = MagicMock(side_effect=lambda k,d=None: session_items.pop(k,d))
    with patch("starlette.requests.Request.session", mock_session_obj):
        response = client.get(f"{app_settings.API_V1_STR}/login/google/callback?code=auth_code&state=test_state_value", follow_redirects=False)
    ...
    assert "error=server_error" in redirect_location and "Failed to get token" in redirect_location

@patch("app.api.routes.google_oauth.settings")
@patch("app.api.routes.google_oauth.requests")
def test_google_callback_user_info_fails(mock_requests: MagicMock, mock_route_settings: MagicMock, client: TestClient):
    mock_route_settings.configure_mock(**app_settings.model_dump(exclude_none=True)); mock_route_settings.GOOGLE_CLIENT_ID = "test_cid"; mock_route_settings.GOOGLE_CLIENT_SECRET = "test_csec"; mock_route_settings.google_oauth_redirect_uri = "http://cb.uri"
    mock_token_resp = MagicMock(); mock_token_resp.json.return_value = {"access_token": "ga_token"}
    mock_requests.post.return_value = mock_token_resp
    mock_requests.get.side_effect = requests.exceptions.HTTPError("Failed to get user info")
    mock_session_obj = MagicMock(); session_items = {"google_oauth_state": "test_state_value"}
    mock_session_obj.get = MagicMock(side_effect=session_items.get); mock_session_obj.pop = MagicMock(side_effect=lambda k,d=None: session_items.pop(k,d))
    with patch("starlette.requests.Request.session", mock_session_obj):
        response = client.get(f"{app_settings.API_V1_STR}/login/google/callback?code=auth_code&state=test_state_value", follow_redirects=False)
    assert response.status_code == 307
    redirect_location = unquote(response.headers["location"])
    assert "error=server_error" in redirect_location and "Failed to get user info" in redirect_location
...
def test_old_google_callback_api_user_info_mismatch(
    mock_route_settings: MagicMock, mock_requests: MagicMock, mock_get_db_patch: MagicMock,
    client: TestClient, mock_db_session_fixture: MagicMock
):
    mock_route_settings.configure_mock(**app_settings.model_dump(exclude_none=True))
    mock_get_db_patch.return_value = mock_db_session_fixture
