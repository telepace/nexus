...
from app.api.routes.prompts import router as prompts_router, _check_prompt_access
from app.core.config import settings as app_settings 
from app.api.deps import get_db, get_current_user
from app.models.prompt import (
    Tag, TagCreate, TagUpdate, 
    Prompt, PromptCreate, PromptReadWithTags, PromptUpdate,
    PromptVersion, PromptVersionCreate, PromptTagLink
)
...
def test_check_prompt_access_superuser(): mock_user=MagicMock(is_superuser=True,id=uuid.uuid4()); mock_prompt=MagicMock(); assert _check_prompt_access(mock_prompt,mock_user) is True
def test_check_prompt_access_creator(): user_id=uuid.uuid4();mock_user=MagicMock(id=user_id,is_superuser=False);mock_prompt=MagicMock(created_by=user_id,visibility="private");assert _check_prompt_access(mock_prompt,mock_user) is True
def test_check_prompt_access_public(): mock_user=MagicMock(id=uuid.uuid4(),is_superuser=False);mock_prompt=MagicMock(created_by=uuid.uuid4(),visibility="public");assert _check_prompt_access(mock_prompt,mock_user) is True
def test_check_prompt_access_team_member(): user_id,team_id=uuid.uuid4(),uuid.uuid4();mock_t=MagicMock();mock_t.id=team_id;mock_user=MagicMock(id=user_id,is_superuser=False,teams=[mock_t]);mock_prompt=MagicMock(created_by=uuid.uuid4(),visibility="team",team_id=team_id);assert _check_prompt_access(mock_prompt,mock_user) is True
def test_check_prompt_access_team_not_member(): user_id,t1,t2=uuid.uuid4(),uuid.uuid4();mock_t=MagicMock();mock_t.id=t1;mock_user=MagicMock(id=user_id,is_superuser=False,teams=[mock_t]);mock_prompt=MagicMock(created_by=uuid.uuid4(),visibility="team",team_id=t2);assert _check_prompt_access(mock_prompt,mock_user) is False
def test_check_prompt_access_private_not_creator_not_superuser(): mock_user=MagicMock(id=uuid.uuid4(),is_superuser=False);mock_prompt=MagicMock(created_by=uuid.uuid4(),visibility="private");assert _check_prompt_access(mock_prompt,mock_user) is False
