...
def test_create_prompt_success_no_tags(MPr: MagicMock, MPV: MagicMock, client: TestClient, mock_db_session_fixture: MagicMock, mock_current_user_fixture: MagicMock):
    p_in = PromptCreate(name="Test", content="Content", type="simple", visibility="public")
...
def test_create_prompt_with_tags(MT: MagicMock, MPr: MagicMock, MPV: MagicMock, client: TestClient, mock_db_session_fixture: MagicMock, mock_current_user_fixture: MagicMock):
    ...
    p_in = PromptCreate(name="Tags", content="Content", type="simple", visibility="public", tag_ids=[t_id1, t_id2])
    p_mock = MagicMock(spec=Prompt, id=uuid.uuid4(), tags=[]); MPr.return_value=p_mock
    for k,v in p_in.model_dump(exclude={"tag_ids"}).items(): setattr(p_mock,k,v)
    p_mock.created_by = mock_current_user_fixture.id; p_mock.content = p_in.content; p_mock.input_vars = p_in.input_vars
    MPV.return_value = MagicMock(spec=PromptVersion)
...
def test_create_prompt_db_error(MPr: MagicMock, client: TestClient, mock_db_session_fixture: MagicMock):
    p_in = PromptCreate(name="Error", content="Err", type="simple", visibility="public"); MPr.return_value=MagicMock(spec=Prompt, id=uuid.uuid4(), tags=[])
...
def test_read_prompts_with_tag_filter(mock_sql_func: MagicMock, mock_sqlmodel_select: MagicMock, client: TestClient, mock_db_session_fixture: MagicMock):
    tag_id1 = uuid.uuid4()
    ...
    # Mock func.count and func.max
    mock_sql_func.count.return_value = "count_mock"
...
def test_read_prompt_success(client: TestClient, mock_db_session_fixture: MagicMock, mock_current_user_fixture: MagicMock):
    p_id = uuid.uuid4(); mock_p = Prompt(id=p_id, name="Test", content="Test", type="simple", created_by=mock_current_user_fixture.id, visibility="private", tags=[])
...
def test_read_prompt_access_denied(mca:MagicMock, client: TestClient, mock_db_session_fixture: MagicMock):
    p_id = uuid.uuid4(); mock_p = Prompt(id=p_id, name="Test", content="Test", type="simple", created_by=uuid.uuid4())
    mock_db_session_fixture.get.return_value = mock_p
    response = client.get(f"{app_settings.API_V1_STR}/{p_id}")
...
def test_update_prompt_success_change_name_and_tags(
    ...
    existing_prompt = Prompt(id=prompt_id, name="Old Name", content="Old Content", type="simple", created_by=mock_current_user_fixture.id, visibility="private", tags=[Tag(id=uuid.uuid4(), name="OldTag")])
    ...
    if update_payload.get("tag_ids"):
        update_payload["tag_ids"] = [str(tag_id) for tag_id in update_payload["tag_ids"]]
...
@patch("app.api.routes.prompts.datetime")
@patch("app.api.routes.prompts.select")
def test_create_prompt_version_success(mock_select: MagicMock, mock_dt: MagicMock, mock_func: MagicMock, client: TestClient, mock_db_session_fixture: MagicMock, mock_current_user_fixture: MagicMock):
    p_id=uuid.uuid4()
    mock_p=Prompt(
        id=p_id,
        name="Test Prompt",
