...
from app.utils.background_tasks import process_content_task
...
class TestAIProcessingStatusBug:
    ...
    @pytest.mark.asyncio
    async def test_status_not_completed_before_ai_processing(self, db_session: Session):
        """
        ...
        async def mock_ai_initialization_layer(*args, **kwargs):
        ...
        with patch('app.utils.background_tasks.get_content_processor', return_value=mock_processor), \
    ...
    @pytest.mark.asyncio
    async def test_processing_stages_order(self, db_session: Session):
        """
        ...
        # Mock AI pipeline
        async def mock_ai_processing(*args, **kwargs):
        ...
        assert "processing" in status_changes, "Should have 'processing' status during processing"
        
        # The critical assertion: "completed" should only appear at the END
        completed_indices = [i for i, status in enumerate(status_changes) if status == "completed"]
    ...
    @pytest.mark.asyncio 
    async def test_ai_processing_after_basic_processing(self, db_session: Session):
        """
        ...
        def mock_basic_processing(*args, **kwargs):
            processing_order.append("basic_processing_complete")
            result = Mock()
            result.success = True
            result.markdown_content = "Processed"
            result.metadata = {}
            return result
        
        # Mock AI processor  
        async def mock_ai_processing(*args, **kwargs):
            processing_order.append("ai_processing_started")
        ...
        assert processing_order == ["basic_processing_complete", "ai_processing_started"], \
            f"BUG: Wrong processing order: {processing_order}"
...
if __name__ == "__main__":
    ...
    pytest.main([__file__ + "::TestAIProcessingStatusBug::test_status_not_completed_before_ai_processing", "-v"])