...
class DeepResearchService:
    ...
    async def _conduct_research(
        ...
        try:
            ...
            researcher = GPTResearcher(
                query=query, report_type=report_type, config_path=None, **config
            )
    ...
    async def _create_content_item_for_research(
        self, query: str, research_report: str, user_id: int
    ) -> ContentItem:
        """为研究报告创建内容项"""
        try:
            # 创建临时markdown文件
            ...
            try:
                ...
                logger.info(
                    f"研究报告处理完成，创建了内容项 ID: {result.content_item.id}"
                )
            ...
            raise HTTPException(
                status_code=500, detail=f"创建研究报告内容项失败: {str(e)}"
            )

    async def process_deep_research(
        self, request: DeepResearchRequest, user_id: int
        ...
        try:
            ...
            content_item = await self._create_content_item_for_research(
                query=request.query, research_report=research_report, user_id=user_id
            )
            ...
            return DeepResearchResponse(
                success=False, error=str(e), message="深度研究失败"
            )
