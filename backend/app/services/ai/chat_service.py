...
class ChatService:
    ...
    def get_model_for_template(self, template_name: str) -> str:
        ...
        if task_name and task_name in resolved_models:
            ...
            logger.info(
                f"Model for template '{template_name}' (task: {task_name}): {model}"
            )
            return model
        else:
            # 回退到全局默认
    ...
    def _parse_jsonl_content(self, content: str) -> list[dict[str, Any]]:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            try:
                ...
                # 将无效JSON行包装为段落块
                blocks.append(
                    {"type": "p", "content": line, "mapping": f"error_{line_num}"}
                )
    ...
    def _format_jsonl_output(self, blocks: list[dict[str, Any]]) -> str:
        ...
        for block in blocks:
            try:
                ...
                lines.append(
                    json.dumps(
                        fallback_block, ensure_ascii=False, separators=(",", ":")
                    )
                )
    ...
    async def generate_with_template(
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    try:
                        ...
                        if json_content.startswith(
                            "```jsonl"
                        ) or json_content.startswith("```json"):
                            ...
                            code_block_pattern = r"```(?:jsonl|json)?\s*\n?(.*?)\n?```"
                            match = re.search(
                                code_block_pattern, json_content, re.DOTALL
                            )
                            if match:
                    ...
                    try:
                        ...
                        if extracted_content.startswith(
                            "```jsonl"
                        ) or extracted_content.startswith("```json"):
                            # 提取```json/```jsonl和```之间的内容
                            # 注意：必须先检查jsonl，因为```jsonl也会匹配```json
                            ...
                            code_block_pattern = r"```(?:jsonl|json)?\s*\n?(.*?)\n?```"
                            match = re.search(
                                code_block_pattern, extracted_content, re.DOTALL
                            )
                            if match:
                        ...
                        if self._is_jsonl_content(extracted_content):
                            logger.info(
                                f"✅ Detected JSONL content for {template_name}"
                            )
                            try:
                                blocks = self._parse_jsonl_content(extracted_content)
                                logger.info(
                                    f"✅ Parsed {len(blocks)} JSONL blocks for {template_name}"
                                )
                                ...
                                logger.warning(
                                    f"JSONL parsing failed for {template_name}: {jsonl_err}"
                                )
                                # 继续到文本包装处理
                        ...
                        logger.warning(
                            f"Code block extraction failed for {template_name}: {e}"
                        )
                        # 回退到原有逻辑
                        result = {"summary": {"text": ai_content}}
                        logger.info(
                ...
                logger.error(f"💥 LiteLLM proxy调用失败: {lite_err}; 回退到Mock数据")
    ...
    async def _call_litellm_proxy(
        ...
        def sync_request():
            ...
            logger.info(
                f"🎯 使用配置的token限制: {max_tokens} (模型: {selected_model})"
            )
            ...
            for attempt in range(max_retries):
                try:
                    logger.info(
                        f"🚀 LiteLLM API调用 (尝试 {attempt + 1}/{max_retries}): {selected_model}"
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(
                            f"✅ LLM response received successfully, content length: {len(content)}"
                        )
                        logger.debug(
                            f"🔍 LLM response content preview: {content[:200]}"
                        )
                        ...
                        logger.error(
                            f"❌ LiteLLM API HTTP {response.status_code}: {error_text}"
                        )
                    ...
                    if attempt < max_retries - 1:
                        ...
                        raise Exception(
                            f"LLM API timeout after {max_retries} attempts: {e}"
                        )
                    ...
                    if attempt < max_retries - 1:
                        ...
                        raise Exception(
                            f"LLM API connection error after {max_retries} attempts: {e}"
                        )
                    ...
                    if attempt < max_retries - 1:
                        ...
                        raise Exception(
                            f"LLM API unknown error after {max_retries} attempts: {e}"
                        )
