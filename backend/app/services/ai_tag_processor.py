...
class AITagProcessor:
    ...
    def process_and_create_tags_from_ai_result(
        ...
        logger.info(
            f"✅ 标签处理完成: 成功处理 {len(tags)} 个标签 [{', '.join([tag.name for tag in tags])}]"
        )
        return tags
    ...
    def _extract_tags_from_ai_result(self, ai_result: dict[str, Any]) -> list[str]:
                ...
                if isinstance(tags_data, list):
                    ...
                    if "tags" in tags_data:
                        if isinstance(tags_data["tags"], list):
                            tag_names.extend(
                                [str(tag).strip() for tag in tags_data["tags"] if tag]
                            )
        ...
        # 去重并过滤空标签
        unique_tags = list({tag for tag in tag_names if tag.strip()})
    ...
    def _create_content_item_tag_associations(
        ...
        for tag in tags:
            try:
                # 检查是否已存在关联
                existing = (
                    session.query(ContentItemTag)
                    .filter_by(
                        content_item_id=uuid.UUID(content_item_id), tag_id=tag.id
                    )
                    .first()
                )
                ...
                if not existing:
                    ...
                    logger.debug(
                        f"🔗 创建内容-标签关联: {content_item_id} <-> {tag.name}"
                    )
                else:
                    logger.debug(
                        f"⚠️ 内容-标签关联已存在: {content_item_id} <-> {tag.name}"
                    )

            except Exception as e:
                logger.error(
                    f"❌ 创建内容-标签关联失败: {content_item_id} <-> {tag.name}, 错误: {str(e)}"
                )
