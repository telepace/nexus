...
from app.models.user import User # To associate with owner_id, though owner_id is passed directly

# Schema imports - assuming these exist
from app.schemas.content import ContentItemCreate, ContentItemUpdate
# Image processing imports
from app.utils.image_processor import process_web_image, process_base64_image
...
async def _process_markdown_images(
    ...
    for match in image_pattern.finditer(content_markdown):
        ...
        new_image_markdown = original_markdown_image # Default to original if processing fails

        try:
            image_schema: Optional[ImageCreate] = None
            if url.startswith("data:image"):
                ...
                if storage_service.public_url and url.startswith(storage_service.public_url): # type: ignore
            ...
            if image_schema:
                ...
                elif db_image.source_url: # Image is linked (web image with keep_link strategy)
...
async def get_content_item(db: AsyncSession, id: uuid.UUID) -> Optional[ContentItem]:
...
async def create_content_item(
    ...
    *,
    ...
    if "title" not in content_data or content_data["title"] is None:
        content_data["title"] = "Untitled Content" # Provide a default or handle as error
...
async def delete_content_item(db: AsyncSession, id: uuid.UUID) -> Optional[ContentItem]:
...
def get_content_assets_by_item_id(
    ...
    statement = (
        select(ContentAsset) # This select is sqlalchemy.future.select, might need adjustment if session is not AsyncSession
