...
export const contentApi = {
  ...
  async getContentMarkdown(id: string): Promise<MarkdownContent> {
    const response = await client.get<MarkdownContent>(`/content/${id}/markdown`)
    return response
  },
   ...
   */
  async getContentChunks(id: string, page: number = 1, size: number = 10): Promise<ContentChunksResponse> {
    const response = await client.get<ContentChunksResponse>(`/content/${id}/chunks?page=${page}&size=${size}`)
    return response
  },
  ...
  async getContentChunksSummary(id: string): Promise<ContentChunksSummary> {
    const response = await client.get<ContentChunksSummary>(`/content/${id}/chunks/summary`)
    return response
  },
  ...
  async createContentItem(data: {
    ...
    const response = await client.post<ContentItemPublic>('/content/create', data)
    return response
  },
  ...
  async processContentItem(id: string): Promise<ContentItemPublic> {
    const response = await client.post<ContentItemPublic>(`/content/process/${id}`)
    return response
  },
