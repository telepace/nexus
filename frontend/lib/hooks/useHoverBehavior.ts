...
// Hook 实现
export const useHoverBehavior = (options: HoverBehaviorOptions): HoverBehaviorReturn => {
  const {
    refId,
    contentId,
    hoverDelay,
  ...
  const showPreview = useCallback(() => {
    ...
    onHoverStart?.(refId)
    onPreviewShow?.(refId)
  }, [disabled, clearTimeouts, log, refId, contentId, setHoveredRef, onHoverStart, onPreviewShow])
