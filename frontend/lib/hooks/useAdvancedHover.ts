...
export const useAdvancedHover = (options: AdvancedHoverOptions) => {
  ...
  const predictHoverIntent = useCallback(
    (mousePos: MousePosition, elementRect: DOMRect): HoverIntent => {
      ...
      if (mouseHistory.length >= 3) {
        ...
        const toCenterMagnitude = Math.sqrt(
          toCenter.x * toCenter.x + toCenter.y * toCenter.y,
        );

        const cosAngle =
          dotProduct / (velocityMagnitude * toCenterMagnitude || 1);
  ...
  const calculateAdaptiveDelay = useCallback(
    (intent: HoverIntent) => {
      ...
      if (intent.speed > speedThreshold) {
        ...
        delay = Math.min(
          maxDelay,
          baseDelay + (intent.speed - speedThreshold) * 0.5,
        );
      } else {
        // 慢速移动时减少延迟，提升响应性
        delay = Math.max(
          minDelay,
          baseDelay - (speedThreshold - intent.speed) * 0.3,
        );
      }
  ...
  const calculateMagnetEffect = useCallback(
    (mousePos: MousePosition, elementRect: DOMRect) => {
  ...
  const handleMouseMove = useCallback(
    (event: MouseEvent) => {
      ...
      if (elementRef.current && enableIntentPrediction) {
        ...
        const isInMagnetRange =
          magnetPos.x !== newPosition.x || magnetPos.y !== newPosition.y;
