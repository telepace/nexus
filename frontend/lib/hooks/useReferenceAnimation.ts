...
// Hook 实现
export const useReferenceAnimation = (
  options: UseReferenceAnimationOptions,
  ...
  // 根据配置生成动画值
  const generateAnimationValue = useCallback(
    (property: keyof AnimationConfig, phase: "from" | "to") => {
  ...
  const variants: AnimationVariants = useCallback(() => {
    ...
    if (
      ...
      (shouldReduceMotion && mergedConfig.respectMotionPreference)
    ) {
      log("动画被禁用或用户偏好减少动画");
      return {
        initial: {},
