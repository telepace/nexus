...
export const useResponsiveReference = (options: ResponsiveReferenceOptions) => {
  ...
  // 基于设备能力确定设备类型
  const determineDeviceType = useCallback((caps: DeviceCapabilities): DeviceType => {
  ...
  const applyAdaptiveConfiguration = useCallback((
    ...
    switch (deviceType) {
      case 'mobile':
        adaptiveConfig = {
          ...adaptiveConfig,
          hoverDelay: mobileHoverMode === 'disabled' ? 0 : 
                     mobileHoverMode === 'long-press' ? 500 : 150,
          hideDelay: 100,
      ...
      case 'tablet':
        ...
        adaptiveConfig = {
          ...adaptiveConfig,
          hoverDelay: mode === 'mobile' ? 300 : 
                     mode === 'desktop' ? 150 : 200,
          hideDelay: 150,
  ...
  }, [enableGestureRecognition, enableTouchOptimization, mobileHoverMode, onGestureDetected, log])
  ...
  const handleTouchMove = useCallback((event: TouchEvent) => {
    ...
    if (distance > 10 && touchState.isLongPress) {
      setTouchState(prev => prev ? { ...prev, isLongPress: false } : null)
    }
  ...
  }, [detectDeviceCapabilities, determineDeviceType, applyAdaptiveConfiguration, onDeviceTypeDetected, log])
  ...
  }, [deviceType, detectDeviceCapabilities, determineDeviceType, applyAdaptiveConfiguration, onDeviceTypeDetected, log])
  ...
  }, [enableTouchOptimization, capabilities?.hasTouch, handleTouchStart, handleTouchMove, handleTouchEnd])
  ...
  const getOptimalInteractionMode = useCallback(() => {
    ...
    switch (deviceType) {
      ...
      case 'tablet':
        return tabletHoverMode === 'hybrid' ? 'hover-and-touch' : tabletHoverMode
      case 'desktop':
