...
export const usePerformanceOptimizedReference = (
  options: PerformanceOptimizedReferenceOptions,
) => {
  const {
    contentId,
  ...
  // 预加载逻辑
  const preloadReferences = useCallback(
    (items: VirtualItem[]) => {
  ...
  useEffect(() => {
    ...
    frameSchedulerManager.schedule(() => {
      const newVirtualItems = calculateVirtualItems(
        totalItems,
        scrollTop,
        viewportHeight,
      );
  ...
  useEffect(() => {
    ...
    const updateMetrics = () => {
      ...
      if (deltaTime >= 1000) {
        // 每秒更新一次
        const frameRate =
          performanceObserver.current.frameCount / (deltaTime / 1000);
        const avgRenderTime =
          performanceObserver.current.renderTimes.reduce((a, b) => a + b, 0) /
