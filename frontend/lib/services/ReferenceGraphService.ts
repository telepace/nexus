...
export interface ReferenceEdge {
  ...
  // 关系类型
  type: 'citation' | 'similarity' | 'sequence' | 'contradiction' | 'elaboration'
...
class ReferenceGraphService {
  ...
  private recommendationCache: Map<string, ReferenceRecommendation[]> = new Map()
  ...
  private analyzeContent(content: string): {
    ...
    const positiveWords = ['good', 'great', 'excellent', 'amazing', 'wonderful', 'positive', 'benefit', 'advantage']
    const negativeWords = ['bad', 'terrible', 'awful', 'negative', 'problem', 'issue', 'disadvantage', 'concern']
  ...
  private identifyTopics(content: string): string[] {
    // 主题关键词映射
    const topicKeywords = {
      'technology': ['ai', 'artificial', 'intelligence', 'machine', 'learning', 'algorithm', 'data', 'computer', 'software', 'digital'],
      'science': ['research', 'study', 'experiment', 'hypothesis', 'theory', 'analysis', 'scientific', 'evidence', 'method'],
      'business': ['market', 'company', 'revenue', 'profit', 'strategy', 'customer', 'product', 'service', 'management'],
      'education': ['learning', 'student', 'teacher', 'education', 'knowledge', 'skill', 'training', 'course', 'curriculum'],
      'health': ['health', 'medical', 'disease', 'treatment', 'patient', 'doctor', 'medicine', 'therapy', 'wellness'],
      'environment': ['environment', 'climate', 'sustainability', 'green', 'renewable', 'pollution', 'conservation', 'ecology']
    }
  ...
  private calculateSemanticSimilarity(content1: string, content2: string): number {
    ...
    // 计算交集
    const commonWords = new Set([...vector1.keys()].filter(word => vector2.has(word)))
  ...
  private discoverRelationships(nodeId: string): void {
    ...
    this.graph.nodes.forEach((otherNode, otherNodeId) => {
      ...
      // 语义相似度
      const similarity = this.calculateSemanticSimilarity(node.content, otherNode.content)
      ...
      // 序列关系（基于位置）
      if (node.contentId === otherNode.contentId && 
          node.position && otherNode.position) {
        const distance = Math.abs(node.position.paragraph - otherNode.position.paragraph)
        if (distance === 1) {
      ...
      // 主题相关性
      const commonTopics = node.tags.filter(tag => otherNode.tags.includes(tag))
      if (commonTopics.length > 0) {
        const topicSimilarity = commonTopics.length / Math.max(node.tags.length, otherNode.tags.length)
        if (topicSimilarity > 0.3) {
          this.addReferenceEdge(nodeId, otherNodeId, 'elaboration', topicSimilarity, {
  ...
  private updateGraphStats(): void {
    ...
    this.graph.stats = {
      ...
      stronglyConnectedComponents: this.findStronglyConnectedComponents().length
    }
  ...
  private calculateNodeMetrics(): void {
    ...
    this.graph.edges.forEach(edge => {
      degreeCentrality.set(edge.sourceId, (degreeCentrality.get(edge.sourceId) || 0) + 1)
      degreeCentrality.set(edge.targetId, (degreeCentrality.get(edge.targetId) || 0) + 1)
    })
    ...
    this.graph.nodes.forEach((node, nodeId) => {
      ...
      node.importance = (node.centrality * 0.4 + qualityWeight * 0.3 + accessWeight * 0.3)
    })
  }
  ...
  getSmartRecommendations(
    ...
    this.graph.nodes.forEach((node, nodeId) => {
      ...
      const commonTopics = currentTopics.filter(topic => node.tags.includes(topic))
      if (commonTopics.length > 0) {
        const topicScore = commonTopics.length / Math.max(currentTopics.length, node.tags.length)
    ...
    const uniqueRecommendations = recommendations
      .filter((rec, index, arr) => 
        arr.findIndex(r => r.refId === rec.refId && r.contentId === rec.contentId) === index
      )
    ...
    this.graph.nodes.forEach((node, nodeId) => {
      ...
      if (clusterNodes.length > 1) {
        ...
        const centerNodeId = clusterNodes.reduce((center, nodeId) => {
          ...
          return currentNode.importance > centerNode.importance ? nodeId : center
        })
  ...
  private calculateClusterCoherence(nodeIds: string[]): number {
    ...
    for (let i = 0; i < nodeIds.length; i++) {
      for (let j = i + 1; j < nodeIds.length; j++) {
        ...
        const similarity = this.calculateSemanticSimilarity(node1.content, node2.content)
  ...
  private getTopCategories(nodes: ReferenceNode[]): Array<{category: string, count: number}> {
   ...
   * 📤 导出图谱数据
   */
  exportGraph(): {nodes: ReferenceNode[], edges: ReferenceEdge[], stats: typeof this.graph.stats} {
    return {
