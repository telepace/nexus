...
class ContentDataManager {
  ...
  cleanup(): void {
    ...
    for (const [key, data] of this.cache.entries()) {
      const ttl = key.includes(":preview:")
        ? this.PREVIEW_CACHE_TTL
        : this.CACHE_TTL;
  ...
  private getCachedData(
    ...
    const ttl =
      options.mode === "preview" ? this.PREVIEW_CACHE_TTL : this.CACHE_TTL;
  ...
  private async fetchData(
    ...
    if (options.includeConversations && results.length > 1) {
      ...
      if (conversationsResult.status === "fulfilled") {
        conversations =
          (conversationsResult.value as ConversationListResponse)
            .conversations || [];
      } else {
        console.warn(
