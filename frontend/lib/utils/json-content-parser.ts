...
export const isJsonContent = (content: string): boolean => {
  ...
  if (lines.length > 1) {
    ...
    return firstLine.startsWith('{') && firstLine.includes('"t"') && firstLine.includes('"c"');
  }
...
export const cleanMarkdownCodeBlock = (content: string): string => {
  const trimmed = content.trim();
  // 注意：jsonl 必须在 json 前面，避免 jsonl 被误匹配为 json
  const jsonBlockRegex = /^```(?:jsonl|json|typescript|javascript)?\s*\n?([\s\S]*?)\n?```$/;
 ...
 * @returns 处理结果
 */
export const batchParseJsonContent = (contents: string[]): {
