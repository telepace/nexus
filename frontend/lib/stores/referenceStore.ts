...
export const useReferenceStore = create<ReferenceState>()(
  subscribeWithSelector((set, get) => ({
    ...
    openModal: (refId, position = { x: window.innerWidth / 2, y: window.innerHeight / 2 }) => {
      set({ 
        activeModal: refId, 
...
// 便捷的状态选择器
export const useReferenceConfig = () => useReferenceStore((state) => state.config)
export const useHoveredRef = () => useReferenceStore((state) => state.hoveredRef)
export const useActiveModal = () => useReferenceStore((state) => state.activeModal)
export const useReferenceCache = () => useReferenceStore((state) => ({
