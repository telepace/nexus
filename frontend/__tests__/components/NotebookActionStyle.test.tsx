...
// Mock dependencies
const MockMarkdownRenderer = ({ content, inline }: { content: string; inline?: boolean }) => (
  <span data-testid="markdown-renderer">{content}</span>
);

const MockEnhancedReferenceIndicator = ({ references }: { references: number[] }) => (
...
jest.mock('@/components/ui/NeumorphicExpandButton', () => ({
  NeumorphicExpandButton: ({ onExpand, side }: { onExpand?: () => void; side?: string }) => (
    <button 
...
describe('Notebook Action Style', () => {
  ...
  it('renders content with markdown renderer', () => {
    ...
    expect(screen.getByTestId('markdown-renderer')).toHaveTextContent('Test action content');
  });
  ...
  it('does not show expand button when expandable is false', () => {
    ...
    expect(screen.queryByTestId('neumorphic-expand-button')).not.toBeInTheDocument();
  });
  ...
  it('shows expand button when expandable is true', () => {
    ...
    expect(screen.getByTestId('neumorphic-expand-button')).toHaveAttribute('data-side', 'left');
  ...
  it('shows reference indicator when has references', () => {
    ...
    expect(screen.getByTestId('reference-indicator')).toHaveTextContent('Refs: 1,2,3');
  });
  ...
  it('handles alternative block property names', () => {
    ...
    expect(screen.getByTestId('markdown-renderer')).toHaveTextContent('Alternative content format');
  });
