...
export function LanguageDebugPanel() {
  ...
  useEffect(() => {
    ...
    const info = {
      ...
      localStorageLanguage: typeof localStorage !== 'undefined' ? localStorage.getItem('preferred-language') : null,
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div>浏览器语言: <Badge variant="outline">{debugInfo.navigatorLanguage}</Badge></div>
            <div>检测到的语言: <Badge variant="outline">{debugInfo.detectedLocale}</Badge></div>
            <div>本地存储: <Badge variant="outline">{debugInfo.localStorageLanguage || '未设置'}</Badge></div>
            <div>API语言参数: <Badge variant="outline">{debugInfo.detectedLocale === 'en' ? 'English' : 'Chinese'}</Badge></div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1">
            {debugInfo.navigatorLanguages?.map((lang: string, index: number) => (
