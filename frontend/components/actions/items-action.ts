...
export const fetchItems = cache(async (): Promise<FetchItemsReturn> => {
  ...
  if (itemsCache && (now - lastFetchTime < CACHE_TTL)) {
    console.log(`[${requestId}] 返回缓存的物品数据，缓存时间: ${new Date(lastFetchTime).toISOString()}`);
    return itemsCache;
  }
  ...
  try {
    console.log(`[${requestId}] 获取物品数据，token: ${token.substring(0, 5)}...`);
    ...
    if (error) {
      ...
      const errorResult = {
        error: typeof error === "string" ? error : error ? JSON.stringify(error) : "未知API错误",
        status: 500
      };
      itemsCache = errorResult;
      lastFetchTime = now;
    ...
    console.log(`[${requestId}] fetchItems 执行完成，数据${Array.isArray(result) ? `长度: ${result.length}` : '是错误对象'}`);
    return result;
  } catch (error) {
...
export async function addItem(prevState: {}, formData: FormData) {
  ...
  try {
    ...
    if (error) {
      ...
      return {
        message: typeof error === "string" ? `添加物品失败: ${error}` : `添加物品失败: ${JSON.stringify(error)}`,
      };
    }
