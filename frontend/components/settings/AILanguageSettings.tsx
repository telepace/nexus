...
export function AILanguageSettings({ className }: AILanguageSettingsProps) {
  ...
  const updateAILanguage = async (language: string) => {
    ...
    try {
      ...
      // 更新本地状态
      setSettings(prev => prev ? { ...prev, ai_output_language: language } : null);
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            💡 此设置将影响所有 AI 功能的输出语言，包括内容分析、摘要生成、标签提取等
          </p>
        </div>
          ...
          <div className="space-y-2">
            <label className="flex items-center justify-between">
              ...
              <input
                ...
                onChange={(e) => updateSettings({ auto_generate_summary: e.target.checked })}
                disabled={saving}
                className="rounded"
              />
            </label>
            ...
            <label className="flex items-center justify-between">
              ...
              <input
                ...
                onChange={(e) => updateSettings({ auto_generate_key_points: e.target.checked })}
                disabled={saving}
                className="rounded"
              />
            </label>
            ...
            <label className="flex items-center justify-between">
              ...
              <input
                ...
                onChange={(e) => updateSettings({ auto_generate_labels: e.target.checked })}
                disabled={saving}
                className="rounded"
              />
            </label>
