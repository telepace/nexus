...
export const SeamlessContentRenderer: React.FC<
  SeamlessContentRendererProps
> = ({
  contentId,
  className = "",
  initialChunkSize = 15,
  ...
  const loadInitialChunks = useCallback(async () => {
    try {
      ...
      setLoadingProgress(
        Math.min(
          (response.chunks.length / (response.pagination?.total_chunks || 1)) *
            100,
          90,
        ),
      );
  ...
  const renderVirtualChunkItem = useCallback(
  ...
  return (
    <div
      className={`relative w-full h-full flex flex-col content-area ${className}`}
    >
