    ...
    case "insight":
      return (
        <div className="my-3 rounded-md border-l-4 border-blue-500 bg-blue-50 p-3 select-text">
          <strong className="text-blue-600 text-sm font-medium mr-2">💡 洞察:</strong>
          <MarkdownRenderer content={String(c)} inline={true} />
          {hasReferences && (
            <EnhancedReferenceIndicator references={references} className="ml-2" />
          )}
    ...
    case "concept":
      return (
        <div className="my-3 rounded-md border-l-4 border-purple-500 bg-purple-50 p-3 select-text">
          <strong className="text-purple-600 text-sm font-medium mr-2">🎯 概念:</strong>
          <MarkdownRenderer content={String(c)} inline={true} />
          {hasReferences && (
            <EnhancedReferenceIndicator references={references} className="ml-2" />
          )}
    ...
    case "action":
      return (
        <div className="my-3 rounded-md border-l-4 border-green-500 bg-green-50 p-3 select-text">
          <strong className="text-green-600 text-sm font-medium mr-2">⚡ 行动:</strong>
          <MarkdownRenderer content={String(c)} inline={true} />
          {hasReferences && (
            <EnhancedReferenceIndicator references={references} className="ml-2" />
          )}
