...
import {
  ...
  CompactReferenceTooltip,
} from "./EnhancedReferenceTooltip";
 ...
 * - 多种显示风格
 */
export const ReferenceHyperlinkRenderer: React.FC<
  ReferenceHyperlinkRendererProps
> = ({
  refString,
  className = "",
  variant = "default",
  ...
  const handleReferenceClick = useCallback(
    (refId: number, event: React.MouseEvent) => {
  ...
  return (
    ...
    >
      ...
      {/* 整体描述（屏幕阅读器） */}
      <span className="sr-only">{getReferenceDescription(referenceData)}</span>
    </span>
  );
};
 ...
 * 🎨 预设的引用样式组件
 */
export const InlineReferences: React.FC<
  Pick<ReferenceHyperlinkRendererProps, "refString" | "onReferenceClick">
> = (props) => (
...
export const BadgeReferences: React.FC<
  Pick<ReferenceHyperlinkRendererProps, "refString" | "onReferenceClick">
> = (props) => (
  <ReferenceHyperlinkRenderer
    {...props}
...
export const MinimalReferences: React.FC<
  Pick<ReferenceHyperlinkRendererProps, "refString" | "onReferenceClick">
> = (props) => (
  <ReferenceHyperlinkRenderer
    {...props}
