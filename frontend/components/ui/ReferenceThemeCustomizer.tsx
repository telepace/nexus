...
const ReferenceThemeCustomizer: React.FC<ReferenceThemeCustomizerProps> = ({
  ...
  // 状态管理
  const [currentTheme, setCurrentTheme] = useState<ReferenceTheme>(PRESET_THEMES[0])
  ...
  // 应用主题
  const applyTheme = useCallback((theme: ReferenceTheme) => {
    // 更新 CSS 变量
    ...
    // 尺寸变量
    root.style.setProperty('--ref-indicator-sm', `${theme.sizes.indicator.sm}px`)
    root.style.setProperty('--ref-indicator-md', `${theme.sizes.indicator.md}px`)
    root.style.setProperty('--ref-indicator-lg', `${theme.sizes.indicator.lg}px`)
    
    // 动画变量
    root.style.setProperty('--ref-duration-fast', `${theme.animations.duration.fast}ms`)
    root.style.setProperty('--ref-duration-normal', `${theme.animations.duration.normal}ms`)
    root.style.setProperty('--ref-duration-slow', `${theme.animations.duration.slow}ms`)
  ...
  // 创建自定义主题
  const createCustomTheme = useCallback((base: ReferenceTheme, overrides: Partial<ReferenceTheme>): ReferenceTheme => {
    return {
      ...base,
      ...overrides,
      id: 'custom',
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-1">
              {theme.animations.effects.enableGlow && (
                <Badge variant="secondary" className="text-xs">发光</Badge>
              )}
              {theme.animations.effects.enableBreathe && (
                <Badge variant="secondary" className="text-xs">呼吸</Badge>
              )}
              {theme.accessibility.reducedMotion && (
                <Badge variant="secondary" className="text-xs">减少动画</Badge>
              )}
              {theme.accessibility.highContrast && (
                <Badge variant="secondary" className="text-xs">高对比度</Badge>
              )}
            </div>
          </div>
        </motion.div>
  ...
  const renderAnimationCustomization = () => (
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          {Object.entries(currentTheme.animations.duration).map(([speed, value]) => (
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {Object.entries(currentTheme.animations.effects).map(([effect, enabled]) => (
            <div key={effect} className="flex items-center justify-between">
              <label className="text-sm font-medium">
                {effect.replace(/([A-Z])/g, ' $1').replace('enable ', '').trim()}
              </label>
              <Switch
                checked={enabled}
  ...
  const renderAccessibilityCustomization = () => (
    <Card>
      ...
      <CardContent className="space-y-4">
        {Object.entries(currentTheme.accessibility).map(([setting, enabled]) => (
  ...
  const importTheme = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        const importedTheme = JSON.parse(e.target?.result as string) as ReferenceTheme
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="text-muted-foreground">{currentTheme.metadata.version}</p>
            </div>
            <div>
              <p className="font-medium">作者</p>
              <p className="text-muted-foreground">{currentTheme.metadata.author}</p>
            </div>
            <div>
