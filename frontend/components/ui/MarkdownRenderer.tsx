...
export function MarkdownRenderer({ content, className }: MarkdownRendererProps) {
  if (!content) {
    return <div data-testid="markdown-renderer" className={cn('prose prose-slate dark:prose-invert max-w-none', className)} />
  }

  // 预处理内容，转义可能导致问题的XML标签
  const sanitizedContent = content
    .replace(/<(message|branch|remote|name|url|path|file|tag|version|commit|issue|pr|repo)([^>]*)>/gi, '&lt;$1$2&gt;')
    .replace(/<\/(message|branch|remote|name|url|path|file|tag|version|commit|issue|pr|repo)>/gi, '&lt;/$1&gt;');
    ...
    >
      <ReactMarkdown
        ...
        components={{
          ...
          pre: ({ children, ...props }) => (
            <pre className="mt-6 mb-4 overflow-x-auto rounded-lg bg-muted p-4" {...props}>
              {children}
            </pre>
          ),
