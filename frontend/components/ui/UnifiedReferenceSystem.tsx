...
// 极简引用（只有点击，无悬浮）
export const MinimalReference: React.FC<
  Omit<UnifiedReferenceSystemProps, "variant" | "enableHover">
> = (props) => (
  <UnifiedReferenceSystem {...props} variant="minimal" enableHover={false} />
);

// 标准引用（悬浮 + 点击）
export const StandardReference: React.FC<
  Omit<UnifiedReferenceSystemProps, "variant">
> = (props) => <UnifiedReferenceSystem {...props} variant="standard" />;

// 优雅引用（完整功能）
export const ElegantReference: React.FC<
  Omit<UnifiedReferenceSystemProps, "variant">
> = (props) => <UnifiedReferenceSystem {...props} variant="elegant" />;

// 快速引用（无延迟）
export const QuickReference: React.FC<
  Omit<UnifiedReferenceSystemProps, "hoverDelay">
> = (props) => <UnifiedReferenceSystem {...props} hoverDelay={0} />;
...
export const BatchReferenceSystem: React.FC<BatchReferenceSystemProps> = ({
  ...
  return (
    <span
      className={`inline-flex items-center ${spacingClasses[spacing]} ${className || ""}`}
    >
 ...
 * 🎹 键盘导航增强包装器
 */
export interface KeyboardEnhancedReferenceProps
  extends UnifiedReferenceSystemProps {
  enableArrowNavigation?: boolean;
  groupId?: string;
}

export const KeyboardEnhancedReference: React.FC<
  KeyboardEnhancedReferenceProps
> = ({ enableArrowNavigation = true, groupId, ...props }) => {
  // 这里可以添加键盘导航组的逻辑
  // 例如维护一个引用组，支持方向键在组内导航

  return <UnifiedReferenceSystem {...props} enableKeyboard={true} />;
};
...
export const InlineReferenceProcessor: React.FC<
  InlineReferenceProcessorProps
> = ({ content, contentId, variant = "standard", className }) => {
  // 匹配引用模式：[1], [ref:1], [^1]
  ...
  return <span className={className}>{processContent()}</span>;
};
