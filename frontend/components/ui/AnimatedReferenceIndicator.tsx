 ...
 * - 悬停预览
 */
export const AnimatedReferenceIndicator: React.FC<
  AnimatedReferenceIndicatorProps
> = ({
  references,
  variant = "floating",
  size = "md",
  ...
  const getVariantClasses = (index: number) => {
    ...
    switch (variant) {
      ...
      case "bounce":
        return cn(baseAnimation, "hover:animate-bounce hover:scale-105");
    }
  };
 ...
 * 🌟 预设动画风格组件
 */
export const FloatingReferences: React.FC<
  Pick<AnimatedReferenceIndicatorProps, "references" | "onReferenceClick">
> = (props) => (
  <AnimatedReferenceIndicator {...props} variant="floating" color="blue" />
);

export const PulseReferences: React.FC<
  Pick<AnimatedReferenceIndicatorProps, "references" | "onReferenceClick">
> = (props) => (
  <AnimatedReferenceIndicator {...props} variant="pulse" color="purple" />
);

export const GlowReferences: React.FC<
  Pick<AnimatedReferenceIndicatorProps, "references" | "onReferenceClick">
> = (props) => (
  <AnimatedReferenceIndicator {...props} variant="glow" color="green" />
);

export const BounceReferences: React.FC<
  Pick<AnimatedReferenceIndicatorProps, "references" | "onReferenceClick">
> = (props) => (
