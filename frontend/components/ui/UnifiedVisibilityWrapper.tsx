...
const UnifiedVisibilityWrapper = memo<UnifiedVisibilityWrapperProps>(
  ({
    ...
    return (
      ...
      >
        <Suspense fallback={fallback}>{renderedContent}</Suspense>
      </div>
    );
  },
);
 ...
 * 预设配置的可见性包装器变体
 */
export const PreviewWrapper = memo<
  Omit<UnifiedVisibilityWrapperProps, "className">
>((props) => (
  <UnifiedVisibilityWrapper {...props} className="absolute inset-0" />
));

PreviewWrapper.displayName = "PreviewWrapper";

export const CardWrapper = memo<
  Omit<UnifiedVisibilityWrapperProps, "className" | "enableReferenceManager">
>((props) => (
