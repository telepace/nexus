...
const calculateOptimalPosition = (
  ...
  // 如果首选位置可用，使用它
  if (preferredPosition !== 'auto' && positions[preferredPosition as keyof typeof positions]) {
  ...
  finalPos.x = Math.max(16, Math.min(finalPos.x, viewport.width - previewRect.width - 16))
  finalPos.y = Math.max(16, Math.min(finalPos.y, viewport.height - previewRect.height - 16))
...
export const SmartHoverPreview: React.FC<SmartHoverPreviewProps> = ({
  ...
  const [previewPosition, setPreviewPosition] = useState({ x: 0, y: 0, placement: 'bottom' })
  ...
  // Store 和服务
  const { getCachedContent, setCachedContent, getContentCacheKey } = useReferenceStore()
  ...
  const calculatePosition = useCallback(() => {
    ...
    const optimalPos = calculateOptimalPosition(triggerRect, previewRect, position, offset)
  ...
  }, [isVisible, delay, hideDelay, preventAutoHide, loadReferenceContent, calculatePosition, onVisibilityChange])
  ...
  const renderContent = () => {
    switch (loadingState) {
      ...
      case 'success':
        ...
        const displayContent = referenceContent.content.length > maxLength
        ...
        <Card className={cn(
          ...
          <div className="relative">
            <CardHeader className="pb-3">
              ...
              {/* 位置信息 */}
              {showPosition && loadingState === 'success' && referenceContent?.position && (
          ...
          <div 
            className={cn(
              ...
              {
                'top-full left-1/2 -translate-x-1/2 -translate-y-1/2 border-r-0 border-b-0': previewPosition.placement === 'top',
                'bottom-full left-1/2 -translate-x-1/2 translate-y-1/2 border-l-0 border-t-0': previewPosition.placement === 'bottom',
                'left-full top-1/2 -translate-x-1/2 -translate-y-1/2 border-r-0 border-t-0': previewPosition.placement === 'left',
                'right-full top-1/2 translate-x-1/2 -translate-y-1/2 border-l-0 border-b-0': previewPosition.placement === 'right',
              }
            )}
          />
        </Card>
