 ...
 * - 批量高亮管理
 */
export const SmartReferenceHighlighter: React.FC<
  SmartReferenceHighlighterProps
> = ({
  children,
  references,
  autoHighlight = true,
  ...
  const [highlightedRefs, setHighlightedRefs] = useState<Set<number>>(
    new Set(),
  );
  ...
  const getDynamicStyles = () => {
    ...
    return {
      ...
      borderLeft:
        intensity > 0 ? `3px solid ${color.replace("0.15", "0.6")}` : undefined,
...
export const ReferenceHighlightManager: React.FC<
  ReferenceHighlightManagerProps
> = ({
  children,
