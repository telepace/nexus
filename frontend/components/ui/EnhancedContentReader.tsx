...
export const EnhancedContentReader: React.FC<EnhancedContentReaderProps> = ({
  ...
  const handleJumpToParagraph = useCallback((event: CustomEvent) => {
    ...
    console.log('🔍 EnhancedContentReader: 收到跳转事件', { refId, paragraphId, chunks });
    
    // 查找对应的段落元素
    if (containerRef.current) {
      // 如果是分块模式，查找对应的chunk
      if (chunks) {
        console.log('📊 分块模式：查找chunk', { refId, chunksLength: chunks.length });
        ...
        console.log('🎯 找到的目标chunk:', targetChunk, {
          searchMethod: targetChunk ? 
            (chunks.find(c => c.index === refId) ? 'direct-match' : 
             chunks.find(c => c.index === (refId - 1)) ? 'index-offset' : 'array-index') : 'not-found',
          availableIndexes: chunks.map(c => c.index).slice(0, 5) // 只显示前5个避免日志过长
        });
        
        if (targetChunk) {
          // 方法1: 通过data-chunk-id查找
          let chunkElement = containerRef.current.querySelector(`[data-chunk-id="${targetChunk.id}"]`);
          
          // 方法2: 如果方法1失败，通过data-paragraph-index查找
          if (!chunkElement) {
            chunkElement = containerRef.current.querySelector(`[data-paragraph-index="${targetChunk.index}"]`);
          }
          
          // 方法3: 如果还是没找到，尝试通过nth-child查找
          if (!chunkElement) {
            const allChunkElements = containerRef.current.querySelectorAll('.chunk-item, [data-chunk-id], [data-paragraph-index]');
            // 尝试多种索引匹配方式
            const possibleIndexes = [targetChunk.index - 1, refId - 1, targetChunk.index];
          ...
          console.log('🎯 找到的DOM元素:', chunkElement, {
            method: chunkElement ? 
              (chunkElement.hasAttribute('data-chunk-id') ? 'data-chunk-id' : 
               chunkElement.hasAttribute('data-paragraph-index') ? 'data-paragraph-index' : 'nth-child') : 'none',
          ...
          if (chunkElement) {
            ...
            console.log('✅ 跳转成功', { refId, targetChunk, element: chunkElement });
            return;
          } else {
          ...
          console.warn('⚠️ 未找到对应的chunk', { 
            refId, 
            availableChunks: chunks.map(c => ({ id: c.id, index: c.index })).slice(0, 10) // 限制日志长度
          });
        }
      }
      ...
      const paragraphs = containerRef.current.querySelectorAll('p, h1, h2, h3, h4, h5, h6, blockquote, li');
  ...
  useEffect(() => {
    ...
    const paragraphs = containerRef.current.querySelectorAll('p, h1, h2, h3, h4, h5, h6, blockquote, li, .chunk-item');
...
interface EnhancedContentReaderWithProviderProps extends EnhancedContentReaderProps {
  enableReferenceManager?: boolean;
}

export const EnhancedContentReaderWithProvider: React.FC<EnhancedContentReaderWithProviderProps> = ({
