...
export const FavoritePreview = ({ item }: Props) => {
  ...
  const getSummaryContent = (): string | null => {
    ...
    if (aiResult?.summary) {
      ...
      } else if (typeof aiResult.summary === 'object' && aiResult.summary !== null) {
        // 尝试从对象中提取字符串内容
  ...
  const getKeyPoints = (): string[] => {
    ...
    if (aiResult?.key_points) {
      ...
      if (Array.isArray(aiResult.key_points)) {
        return aiResult.key_points.filter(point => typeof point === 'string');
      }
      
      // 如果是对象，尝试提取数组或字符串内容
      if (typeof aiResult.key_points === 'object' && aiResult.key_points !== null) {
      ...
      <div className="flex items-center justify-between h-header px-6 border-b border-border/30 content-glass">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="text-base font-medium text-neutral-900">收藏预览</span>
            <span className="text-xs text-muted-foreground">收藏于 {favoriteTime}</span>
          </div>
        </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-neutral-900 mb-1">原始来源</p>
          ...
          {getSummaryContent() && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-medium text-neutral-900">内容摘要</h3>
              </div>
          ...
          {aiResult?.labels && aiResult.labels.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-medium text-neutral-900">相关标签</h3>
              </div>
