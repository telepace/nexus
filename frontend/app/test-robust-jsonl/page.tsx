            ...
            {testCases.map((testCase) => (
              <Card 
                ...
                className={`cursor-pointer transition-all duration-200 hover:shadow-md ${
                  selectedExample === testCase.id ? 'border-primary shadow-md' : ''
                }`}
                onClick={() => setSelectedExample(selectedExample === testCase.id ? null : testCase.id)}
              >
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="border rounded-lg p-4 bg-muted/20 max-h-96 overflow-y-auto">
                      <RobustJsonlRenderer
                        content={customContent || (selectedExample ? testCases.find(t => t.id === selectedExample)?.content || '' : '')}
          ...
          <CardContent className="text-blue-700 dark:text-blue-300 space-y-3 text-sm">
            <div>
              ...
              <ul className="list-disc list-inside ml-4 mt-1 space-y-1">
                <li>自动修复常见JSON语法错误（单引号、缺失引号、多余逗号等）</li>
