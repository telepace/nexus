...
export const useContentItems = () => {
  ...
  const refetchContentItem = useCallback(
    async (
      contentId: string,
      retryCount = 0,
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (
            ...
            console.warn(
              `Content ${contentId} marked as completed but AI results incomplete, retrying in ${retryDelay}ms...`,
            );
        ...
        console.error(
          `Failed to refetch content item ${contentId} (attempt ${retryCount + 1}):`,
          error,
        );
        ...
        if (
          ...
          console.warn(
            `Retrying refetch for ${contentId} in ${retryDelay}ms...`,
          );
          await new Promise((resolve) => setTimeout(resolve, retryDelay));
          return refetchContentItem(contentId, retryCount + 1);
        }
