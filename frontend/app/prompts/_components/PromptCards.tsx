...
const PromptCard = memo(({ 
  ...
  // 使用 useMemo 缓存统计数据计算
  const stats = useMemo(() => generateConsistentStats(prompt.id), [prompt.id]);
      ...
      <div className="p-4 pt-6">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-200/60">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-600 truncate max-w-[100px]">{authorName}</span>
          </div>
...
export function PromptCards({ prompts, currentUser }: PromptCardsProps) {
  ...
  // 使用 useCallback 优化点击处理函数
  const handleCardClick = useCallback((promptId: string, event: React.MouseEvent) => {
    // 阻止事件冒泡，避免与其他交互元素冲突
    if (
