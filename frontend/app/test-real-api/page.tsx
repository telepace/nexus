...
8:[{"finishReason":"stop"}]`;
  ...
  const renderContent = (content: string, streaming: boolean = false) => {
    ...
    if (isJsonlContent(content)) {
      if (streaming) {
        return <StreamingJsonlRenderer content={content} isLoading={streaming} />;
      } else {
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            ...
            <div>
              <strong>渲染器:</strong> {content ? (isJsonlContent(content) ? "JsonlRenderer" : "MarkdownRenderer") : "无"}
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2 text-sm">
            <div><strong>检测结果:</strong> {content ? (isJsonlContent(content) ? "JSONL格式" : "非JSONL格式") : "无内容"}</div>
            {content && (
              <>
                <div><strong>第一行:</strong> {content.split('\n')[0] || "空"}</div>
                <div><strong>总行数:</strong> {content.split('\n').filter(Boolean).length}</div>
                <div><strong>有效JSON行数:</strong> {
                  content.split('\n').filter(line => {
                    const trimmed = line.trim();
