...
export function useGlobalSSENotifications(
  options: UseGlobalSSENotificationsOptions = {},
) {
  const {
    enabled = true,
    autoReconnect = true,
  ...
  const { user } = useAuth();
  const { handleSSEEvent, setSSEConnected, isSSEConnected } =
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        console.log(
          `${delay / 1000}秒后尝试重连SSE (第${reconnectAttemptsRef.current}次)`,
        );
