...
export function useAIAnalysis(config: AIAnalysisConfig): AIAnalysisReturn {
  ...
  const getCurrentState = useCallback((): AIAnalysisState => {
    ...
    mode, contentId, customContent, customIsLoading, customError, 
    completion, completionIsLoading, completionError, hasStarted, 
    streamContent, retryCount
  ]);

  // 内容分析模式的流式处理 - 修复版本
  const handleContentAnalysis = useCallback(async (instruction: string) => {
    if (!contentId) {
    ...
    try {
      ...
      const finalError = error instanceof Error ? error : new Error("分析失败");
  ...
  // 通用聊天模式处理
  const handleChatAnalysis = useCallback(async (instruction: string) => {
    if (!systemPrompt) {
  ...
  // 开始分析
  const startAnalysis = useCallback(async (instruction: string) => {
    if (mode === "display") {
