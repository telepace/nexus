...
export function useConversationHistory({
  ...
  const saveCachedHistory = useCallback(
    (records: ConversationPublic[]) => {
      // 使用防抖机制，避免频繁写入
  ...
  const loadHistoryFromAPI = useCallback(
    async (force = false) => {
    ...
    [contentId, lastLoadTime, CACHE_KEY],
  ); // 移除errorCount和isBlocked依赖，避免循环
